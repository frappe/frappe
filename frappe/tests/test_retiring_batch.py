...
happening again: one written call, naming the whole batch and the condition that fires it.

These tests keep it accurate. A list nobody checks drifts into a list nobody trusts, and a batch
that has quietly lost half its members is worse than no batch.
...
class TestTheRetiringBatch(IntegrationTestCase):
	...
	def test_every_mechanism_points_at_the_call(self):
		"""The sentence has to be where a successor will read it: in the file they are editing, not
		only in a document they would have to know to look for.
		"""
		for path in BATCH:
	...
	def test_every_path_it_names_still_exists(self):
		"""A batch that has quietly lost members is worse than no batch: the next maintainer reads a
		list, removes what is on it, and leaves behind whatever fell off.
		"""
	...
	def test_each_line_states_a_condition_and_no_version(self):
		"""A version number fires whether or not customers moved, which makes the invitation
		meaningless. Both triggers are conditions a maintainer evaluates.
		"""
	...
	def test_the_accepted_cost_is_on_the_record(self):
		"""Including what was considered instead and why it was not enough, so a successor does not
