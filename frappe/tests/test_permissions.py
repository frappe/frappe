...
from nts.core.doctype.user_permission.user_permission import clear_user_permissions
from nts.core.page.permission_manager.permission_manager import add, remove, reset, update
...
class TestPermissions(ntsTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		nts.clear_cache(doctype="Blog Post")
		user = nts.get_doc("User", "test1@example.com")
		...
		user = nts.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = nts.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		nts.db.delete("User Permission")

		nts.set_user("test1@example.com")

	def tearDown(self):
		nts.set_user("Administrator")
		nts.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def test_select_permission(self):
		...
		nts.clear_cache(doctype="Blog Post")
		nts.set_user("test3@example.com")
		...
		self.assertRaises(nts.PermissionError, post.save)

		permitted_record = nts.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = nts.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = nts.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		nts.set_user("test2@example.com")
		names = [d.name for d in nts.get_list("Blog Post", fields=["name", "blog_category"])]
	...
	def test_user_link_match_doc(self):
		blogger = nts.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		nts.set_user("test2@example.com")

		post = nts.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in nts.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			nts.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = nts.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = nts.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = nts.db.get_value("User", {"name": ("!=", nts.session.user)})
	...
	def test_dont_change_standard_constants(self):
		...
		self.assertRaises(nts.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = nts.get_meta("Blog Post")
		doc = nts.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(nts.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = nts.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = nts.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		nts.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(nts.PermissionError, doc.insert)
	...
	def test_strict_user_permissions(self):
		...
		nts.set_user("Administrator")
		nts.db.delete("Contact")
		nts.db.delete("Contact Email")
		nts.db.delete("Contact Phone")
		...
		allowed_contact = nts.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = nts.get_doc("Contact", "_Test Contact For _Test Supplier")

		nts.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(nts.get_list("Contact")), 2)
		...
		self.assertTrue(len(nts.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = nts.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = nts.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		nts.set_user("test2@example.com")
		self.assertEqual(nts.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in nts.response.docs]
		self.assertTrue(doc.name in doclist)

		nts.set_user("test2@example.com")
		self.assertRaises(nts.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		nts.set_user("test2@example.com")
		self.assertIn(doc.name, nts.get_list("Blog Post", pluck="name"))
		...
		nts.set_user("test2@example.com")
		self.assertNotIn(doc.name, nts.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(nts.PermissionError, nts.delete_doc, "Blog Post", doc.name)

		nts.set_user("test2@example.com")
		nts.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		nts.set_user(current_user)

	def test_child_permissions(self):
		nts.set_user("test3@example.com")
		self.assertIsInstance(nts.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# nts.get_list
		self.assertRaises(nts.PermissionError, nts.get_list, "DefaultValue")
		self.assertRaises(nts.PermissionError, nts.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			nts.PermissionError, nts.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(nts.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if nts.db.exists("Gender", gender):
				continue

			nts.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		nts.db.set_value("User", "test1@example.com", "gender", "Male")
		nts.db.set_value("User", "test2@example.com", "gender", "Female")
		nts.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		nts.set_user("test3@example.com")
		users = nts.get_list("User", pluck="name")
	...
	def test_automatic_permissions(self):
		def assertHasRole(*roles: str | tuple[str, ...]):
			for role in roles:
				self.assertIn(role, nts.get_roles())
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = nts.new_doc("Role", role_name=nts.generate_hash()).insert().name
		with self.set_user("Administrator"):
