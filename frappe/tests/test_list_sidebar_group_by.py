...
class TestListSidebarGroupBy(unittest.TestCase):
	...
	def test_virtual_doctype_assignment_filter_blocked(self, mock_get_meta: MagicMock) -> None:
		...
		mock_meta = frappe._dict({
			'is_virtual': 1,
			'name': 'Test Virtual DocType'
		})
		...
		self.assertFalse(meta and not meta.is_virtual, "Virtual DocType should fail supports_assignments check")
	...
	def test_regular_doctype_assignment_filter_allowed(self, mock_get_meta: MagicMock) -> None:
		...
		self.assertTrue(meta and not meta.is_virtual, "Regular DocType should pass supports_assignments check")
	...
	def test_meta_edge_cases(self, mock_get_meta: MagicMock) -> None:
		...
		supports_assignments = meta and not getattr(meta, 'is_virtual', False)
		self.assertTrue(supports_assignments, "DocType without is_virtual should default to supporting assignments")
