...
QUnit.test("Test modules view", function(assert) {
	...
	frappe.run_serially([
		...
		() => frappe.timeout(0.5),
		() => frappe.tests.click_link('Document Share Report'),
		...
		() => frappe.set_route("modules", "Setup"),
		() => frappe.timeout(0.5),
		() => frappe.tests.click_link('Print Settings'),
		...
		() => frappe.set_route("modules", "Setup"),
		() => frappe.timeout(0.5),
		() => frappe.tests.click_link('Workflow Action'),
		...
		() => frappe.timeout(0.5),
		() => frappe.tests.click_link('Tools'),
		() => frappe.timeout(0.5),
		() => frappe.tests.click_link('To Do'),
