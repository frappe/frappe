...
def create_test_blog_post():
	if not frappe.db.exists("DocType", "Test Blog Post"):
		...
		test_blog_doc.insert(ignore_if_duplicate=True)
...
def create_test_blog_records():
	...
	for r in records:
		frappe.get_doc(r).insert(ignore_if_duplicate=True)


def create_test_blog_category():
	if not frappe.db.exists("DocType", "Test Blog Category"):
		frappe.get_doc(
			{
				"doctype": "DocType",
				"autoname": "field:title",
...
def create_test_blogger():
	if not frappe.db.exists("DocType", "Test Blogger"):
		frappe.get_doc(
			{
				"doctype": "DocType",
				"name": "Test Blogger",
