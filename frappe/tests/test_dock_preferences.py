...
def app_rooted_at(app, root):
	...
	def installed(*args, _ensure_on_bench=False, **kwargs):
		apps = real_installed(*args, _ensure_on_bench=_ensure_on_bench, **kwargs)
		# an invented app has no python package: keep it out of the bench-level list the hooks loader reads
...
def shipped_dock(records: dict[str, list[dict]]):
	...
	real_installed = frappe.get_installed_apps
	real_path = frappe.get_app_path
	invented = [app for app in records if app not in real_installed()]
	...
	def patched_installed(*args, _ensure_on_bench=False, **kwargs):
		apps = real_installed(*args, _ensure_on_bench=_ensure_on_bench, **kwargs)
...
def sorted_modules_of(app: str) -> list[str]:
	...
	skip = set(get_code_only_modules())
