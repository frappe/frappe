...
class TestDocument(IntegrationTestCase):
	...
	# SQLite has no varchar length: Data columns are unbounded TEXT, so no length error
...
class TestGetDocs(IntegrationTestCase):
	...
	# SQLite drops FOR UPDATE (the single write lock already covers the rows), so no flag is set
	@unimplemented_for(db_type_is.SQLITE)
