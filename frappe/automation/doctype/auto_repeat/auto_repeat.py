...
from frappe.utils import cstr, getdate, split_emails, add_days, today, get_last_day, get_first_day, month_diff, now, parser
...
class AutoRepeat(Document):
	...
	def set_dates(self):
		if self.disabled:
			...
			if self.repeat_by_minutes == 1:
				next_schedule_time_temp = datetime.strptime(self.set_start_time, '%Y-%m-%d %H:%M:%S')
				next_schedule_time_temp += relativedelta(minutes=self.set_minutes_interval)
				self.next_schedule_time = next_schedule_time_temp
...
def make_auto_repeat_entry_minutes():
	enqueued_method = 'frappe.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries'
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[frappe.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries_minutes(date)
		frappe.enqueue(enqueued_method, data=data)
...
def create_repeated_entries(data):
	for d in data:
		...
		if doc.next_schedule_time <= parser.parse(current_date_minutes) and doc.next_schedule_time.time() <= parser.parse(current_date_minutes).time()  and not doc.disabled and doc.repeat_by_minutes == 1:
			doc.create_documents()
			if not doc.disabled:
				next_schedule_time_tempp = doc.next_schedule_time
				next_schedule_time_tempp += relativedelta(minutes=doc.set_minutes_interval)
#				doc.next_schedule_time = next_schedule_time_tempp				
				frappe.db.set_value('Auto Repeat', doc.name, 'next_schedule_time', next_schedule_time_tempp)
...
def get_auto_repeat_entries_minutes(date=None):
	if not date:
		date = getdate(today())
	return frappe.db.get_all('Auto Repeat', filters=[
		['repeat_by_minutes', '=', 1],
