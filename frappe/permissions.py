		...
		for field in meta.get_link_fields():
			...
			if allowed_docs and d.get(field.fieldname) not in allowed_docs:
				...
				if d.get("parentfield"):
					# "You are not allowed to access Company 'Restricted Company' in row 3, field Reference Type"
					msg = _("You are not allowed to access {0} '{1}' in row {2}, field {3}").format(
						_(field.options), d.get(field.fieldname) or _("empty"), d.idx, _(field.label)
					)
				else:
					# "You are not allowed to access Company 'Restricted Company' in field Reference Type"
					msg = _("You are not allowed to access {0} '{1}' in field {2}").format(
						_(field.options), d.get(field.fieldname) or _("empty"), _(field.label)
					)
