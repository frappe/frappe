...
from nts.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from nts.contacts.doctype.contact_email.contact_email import ContactEmail
		from nts.contacts.doctype.contact_phone.contact_phone import ContactPhone
	...
	def autoname(self):
		...
		if nts.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			nts.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = nts.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not nts.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not nts.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			nts.throw(_("Only one {0} can be set as primary.").format(nts.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			nts.throw(
				_("Only one {0} can be set as primary.").format(nts.bold(nts.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	nts.response["filename"] = f"{contact.name}.vcf"
	nts.response["filecontent"] = vcard.serialize().encode("utf-8")
...
def download_vcards(contacts: str):
	...
	from nts.utils.data import now

	contact_ids = nts.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = nts.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	nts.response["filename"] = f"{timestamp} Contacts.vcf"
	nts.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	nts.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		nts.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = nts.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	doctype = "Contact"
	if not nts.get_meta(doctype).get_field(searchfield) and searchfield not in nts.db.DEFAULT_COLUMNS:
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = nts.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = nts.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return nts.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from nts.contacts.doctype.address.address import get_condensed_address

	if not nts.has_permission("Contact", "read"):
		return []

	contact_list = nts.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		...
		if contact.address and nts.has_permission("Address", "read"):
			address = nts.get_doc("Address", contact.address)
