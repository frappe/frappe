...
import nts.defaults
from nts import _
from nts.database.utils import (
	DefaultOrderBy,
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
...
from nts.exceptions import DoesNotExistError, ImplicitCommitError
from nts.monitor import get_trace_id
from nts.query_builder import Case
from nts.query_builder.functions import Count
from nts.utils import CallbackManager, cint, get_datetime, get_table_name, getdate, now, sbool
from nts.utils import cast as cast_fieldtype
from nts.utils.deprecations import deprecated, deprecation_warning
...
class Database:
	...
	def __init__(
		...
		self.value_cache = {}
		self.logger = nts.logger("database")
	...
	def sql(
		...
		*,
		...
		Examples:
		        ...
		        nts.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        nts.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        nts.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
			nts.log("Use run method to execute SQL queries generated by Query Builder")
		...
		if trace_id := get_trace_id():
			query += f" /* nts_TRACE_ID: {trace_id} */"

		try:
			self._cursor.execute(query, values)
		except Exception as e:
			if self.is_syntax_error(e):
				nts.log(f"Syntax error in query:\n{query} {values or ''}")

			elif self.is_deadlocked(e):
				raise nts.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise nts.QueryTimeoutError(e) from e
				...
				nts.throw(
					...
					exc=nts.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				nts.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			nts.log(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
		while result := self._transform_result(self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)):
			if pluck:
				...
				for row in result:
					row = nts._dict(zip(keys, row, strict=False))
					if update:
						row.update(update)
					yield row
				...
				nts.throw(_("`as_iterator` only works with `as_list=True` or `as_dict=True`"))
	...
	def _log_query(
		...
		if nts.conf.allow_tests and nts.cache.get_value("flag_print_sql"):
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {
					k: nts.db.escape(v) if isinstance(v, str) else v for k, v in values.items()
				}
			elif isinstance(values, list | tuple):
				return query % tuple(nts.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=None):
		...
		nts.log("--- query explain ---")
		try:
			self._cursor.execute(f"EXPLAIN {query}", values)
		except Exception as e:
			nts.log(f"error in query explain: {e}")
		else:
			nts.log(json.dumps(self.fetch_as_dict(), indent=1))
			nts.log("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = nts.db.sql_list("select name from DocType")
		"""
		...
		if query.lstrip()[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise nts.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, result) -> list[nts._dict]:
		"""Internal. Convert results to dict."""
		if result:
			keys = [column[0] for column in self._cursor.description]

		return [nts._dict(zip(keys, row, strict=False)) for row in result]

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			nts.cache.delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        nts.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        nts.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = nts.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        nts.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = nts.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = nts.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (
						nts.db.is_missing_column(e)
						or nts.db.is_table_missing(e)
						or str(e).startswith("Invalid DocType")
					):
						out = None
					elif (not ignore) and nts.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = nts.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = nts.qb.get_query(
			"Singles",
		...
		if not cast:
			return nts._dict(queried_result)
	...
	def get_all(*args, **kwargs):
		return nts.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return nts.get_list(*args, **kwargs)
	...
	def _get_update_dict(
		...
		if update_modified:
			...
			modified_by = modified_by or nts.session.user
	...
	def set_single_value(
		...
		*,
		...
		Example:
		        ...
		        nts.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
		...
		nts.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data).run(debug=debug)
		nts.clear_document_cache(doctype, doctype)
		...
		if not df:
			nts.throw(
				_("Field {0} does not exist on {1}").format(
					nts.bold(fieldname), nts.bold(doctype), self.InvalidColumnName
				)
			)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		"""
		from nts.model.utils import is_single_doctype
		...
		if isinstance(dn, str):
			nts.clear_document_cache(dt, dn)
		else:
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		nts.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		nts.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = nts.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(nts.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or nts.flags.read_only
	...
	def savepoint(self, save_point):
		...
		Changes can be undone to a save point by doing nts.db.rollback(save_point)
	...
	def a_row_exists(self, doctype):
		...
		return nts.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = nts.cache.get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = nts.qb.get_query(
			table=dt,
			filters=filters,
			fields=Count("*"),
			distinct=distinct,
		...
		if not filters and cache:
			nts.cache.set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = nts.cache.hget("table_columns", table)
		if columns is None:
			...
			if columns:
				nts.cache.hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return nts.get_system_settings(key)
	...
	def is_column_missing(e):
		return nts.db.is_missing_column(e)
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if nts.flags.touched_tables is None:
				nts.flags.touched_tables = set()
			nts.flags.touched_tables.update(tables)
	...
	def bulk_insert(
		...
		*,
		...
		query = nts.qb.into(table).columns(fields)
	...
	def unbuffered_cursor(self):
		...
		Usage:
		        with nts.db.unbuffered_cursor():
		                for row in nts.db.sql("query with huge result", as_iterator=True):
...
def get_query_execution_timeout() -> int:
	...
	if not nts.conf.get("enable_db_statement_timeout"):
		return 0

	# Zero means no timeout, which is the default value in db.
	timeout = 0
	with suppress(Exception):
		if getattr(nts.local, "request", None):
			timeout = nts.conf.http_timeout or 300
