...
def setup_database(force, source_sql, verbose, root_login, root_password, no_mariadb_socket=False):
	...
	root_conn = get_root_db(root_login, root_password)
...
def drop_user_and_database(db_name, root_login, root_password):
	frappe.local.db = get_root_db(root_login, root_password)
...
def get_root_db(root_login, root_password):
	if not frappe.local.flags.root_db:
		if not root_login:
			root_login = frappe.conf.get("root_login") or os.getlogin()
