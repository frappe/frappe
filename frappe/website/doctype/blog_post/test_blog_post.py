...
from nts.utils import random_string, set_request
from nts.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(ntsTestCase):
	def setUp(self):
	...
	def test_generator_not_found(self):
		pages = nts.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{nts.generate_hash(length=5)}"

		nts.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = nts.safe_decode(blog_page_response.get_data())
		...
		cached_value = nts.db.value_cache.get(("DocType", "Blog Post", "name"))
		nts.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = nts.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		nts.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		nts.delete_doc("Blog Post", blog.name)
		nts.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = nts._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			nts.delete_doc(blog.doctype, blog.name)
		nts.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		nts.flags.force_website_cache = True
		print(nts.session.user)
	...
	def test_like_dislike(self):
		...
		nts.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from nts.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not nts.db.exists("Blog Category", category_name):
		nts.get_doc(dict(doctype="Blog Category", title=category_title)).insert()
	if not nts.db.exists("Blogger", "test-blogger"):
		nts.get_doc(dict(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger")).insert()
