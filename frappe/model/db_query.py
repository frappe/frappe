...
class DatabaseQuery(object):
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values.get("docs", []) and not self.doctype in user_permission_values.get("skip_for_doctype", []):
				...
				condition += """`tab{doctype}`.`{fieldname}` in ({values})""".format(
					...
					values=", ".join([('"'+frappe.db.escape(v, percent=False)+'"') for v in user_permission_values.get("docs")]))
