...
class DatabaseQuery:
    ...
    def parse_args(self):
        ...
        for filter_name in ["filters", "or_filters"]:
            ...
            if isinstance(filters, dict):
                fdict = filters
                filters = [
                    make_filter_tuple(self.doctype, key, value) for key, value in fdict.items()
                ]
        ...
        for field in self.fields:
            ...
            if SUB_QUERY_PATTERN.match(field):
                ...
                if function in blacklisted_functions:
                    frappe.throw(
                        _("Use of function {0} in field is restricted").format(function),
    ...
    def extract_tables(self):
        ...
        if self.fields:
            for field in self.fields:
                if not ("tab" in field and "." in field) or any(
                    x for x in sql_functions if x in field
                ):
                    continue
    ...
    def append_link_table(self, doctype, fieldname):
        ...
        self.linked_table_aliases[linked_table.table_alias.replace("`", "")] = (
        ...
        for i, field in enumerate(self.fields):
            if "distinct" in field.lower():
                ...
                if _fn := FN_PARAMS_PATTERN.findall(field):
                    column = (
                        _fn[0].replace("distinct ", "").replace("DISTINCT ", "").replace("`", "")
                    )
                # field: 'distinct name'
                # column: 'name'
                else:
        ...
        for df in doctype_link_fields:
            ...
            if user_permission_values:
                ...
                if docs:
                    ...
                    condition += (
                        cast_name(f"`tab{self.doctype}`.`{df.get('fieldname')}`")
                        + f" in ({values})"
                    )
        ...
        if self.order_by and self.order_by != "KEEP_DEFAULT_ORDERING":
            ...
            if not group_function_without_group_by:
                ...
                if self.doctype_meta.sort_field and "," in self.doctype_meta.sort_field:
                    ...
                    sort_order = (
                        self.doctype_meta.sort_field and self.doctype_meta.sort_order
                    ) or "desc"
                    if self.order_by:
                ...
                if (
                    hasattr(self.doctype_meta, "is_submittable")
                    and self.doctype_meta.is_submittable
                ):
                    if self.order_by:
...
def cast_name(column: str) -> str:
    ...
    if "cast(" not in column.lower() and "::" not in column:
        if LOCATE_PATTERN.search(**kwargs):
            ...
            func = match.groups()[0]
            return re.sub(
                rf"{func}\(\s*([`\"]?name[`\"]?)\s*,", rf"{func}(cast(\1 as varchar),", **kwargs
            )
...
def get_between_date_filter(value, df=None):
    ...
    if fieldtype == "Datetime":
        ...
        to_date = _convert_type_for_between_filters(
            to_date, set_time=datetime.time(23, 59, 59, 999999)
        )
