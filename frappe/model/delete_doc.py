	...
	for link_dt, link_field, issingle in link_fields:
		if not issingle:
			for item in frappe.db.get_values(link_dt, {link_field:doc.name},
				...
				if linked_doctype in ("Communication", "ToDo", "DocShare", "Email Unsubscribe", "File", "Version", "Activity Log", "Document Follow", "Comment"):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	...
	for df in get_dynamic_link_map().get(doc.doctype, []):
		if df.parent in ("Communication", "ToDo", "DocShare", "Email Unsubscribe", "Activity Log", "File", "Version", "View Log", "Document Follow", "Comment"):
			# don't check for communication and todo!
			continue
