...
def bundle(
	...
	# The desk v2 shell, before the esbuild pass. Its manifest is assembled here
	# because `app_prefix` is a Python scalar hook and a prefix cannot be globbed for
	# (#42069) -- and because a singleton conflict must fail before vite starts, which
	# means before this build does any work at all.
...
def build_shell(frappe_app_path: str, production: bool = False):
	"""Assemble the manifest and run the framework's one vite build.

	`bench build --app X` reaches here too, and is accepted-and-ignored rather than
	refused: bench itself passes it (`bench/app.py:948`), and the bundle follows the
	bench's app set rather than any one app (#42069).
	"""
	from frappe.shell.manifest import write as write_manifest

	frontend_path = os.path.join(frappe_app_path, "frontend")
	# Guard on the COMMITTED half. `package.json` is generated by the manifest step and
	# gitignored, so guarding on it would make a fresh clone silently build nothing and
	# then serve "Shell Not Built" at every /apps URL.
	if not os.path.exists(os.path.join(frontend_path, "package.base.json")):
		return

	# Raises SingletonConflict, naming both apps and both ranges. Deliberately not
	# `resolve.dedupe`, which silently picks a winner.
	...
	#
	# vite's `emptyOutDir` forces a choice between two bad outcomes: emptying it means
	# a failed build takes the running site's assets down with it, and not emptying it
	# means every build leaks the last one — content hashes change, nothing is ever
	# removed, and the directory grows without bound (measured: 98 files and 14 MB
	# after a handful of builds, twelve stale copies of the entry chunk).
	#
	# Staging gets both properties at once: a failed build never touches what is being
	# served, and a successful one leaves no orphans behind.
	...
	# Swap with a rename, so a reader mid-request sees one tree or the other and never
