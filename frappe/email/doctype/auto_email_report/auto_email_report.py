	...
	def prepare_dynamic_filters(self):
		...
		if self.use_first_day_of_period:
			...
			if self.dynamic_date_period == "Weekly":
				...
				from_date = get_first_day_of_half_year(from_date)
...
DATE_FORMAT = "%Y-%m-%d"
def get_first_day_of_half_year(as_str=False):
    """
    Returns the first day of the current half-year based on the current date.
    """
    today_date = getdate(today())

    # Determine the half-year (1st or 2nd half)
    half_year = 1 if today_date.month <= 6 else 2

    # Calculate the first day of the half-year
