...
def get_sidebar_stats(stats, doctype, filters=[]):
	...
	tag_counts = frappe.get_all(doctype, filters=filters, fields=["`tabTag Link`.tag", "count(`tabTag Link`.tag) as count"], group_by="`tabTag Link`.tag", debug=True)

	if tag_counts:
		for count in tag_counts:
			if not count.tag == None:
				_user_tags.append([count.tag, count.count])
