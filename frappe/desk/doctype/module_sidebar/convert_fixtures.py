"""Convert an app's old sidebar fixtures into per-module `Module Sidebar` exports.
...
per module under `<app>/<module>/module_sidebar/`, which rides the ordinary doc-files walk with
...
def convert_app(app: str, dry_run: bool = False) -> list[dict]:
	"""Write one `Module Sidebar` export per module the app's fixtures name.
...
def export_path(module: str) -> str:
	...
	`ModuleSidebar.exported_file_path`, which is also what orphan removal derives."""
	scrubbed = frappe.scrub(module)
	return os.path.join(frappe.get_module_path(module), "module_sidebar", scrubbed, f"{scrubbed}.json")
...
def write_export(path: str, module: str, plan, app: str) -> None:
	...
	doc = {
		...
		"items": [
			{
				"doctype": "Module Sidebar Item",
				"parentfield": "items",
				"parenttype": "Module Sidebar",
				"idx": idx,
