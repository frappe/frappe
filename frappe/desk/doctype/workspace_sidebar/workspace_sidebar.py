...
def add_sidebar_items(sidebar_title, sidebar_items):
	"""Save a user's edits to a sidebar (the in-place editor of Workspace Sidebar navigation).

	Outside developer mode the edits land on a per-user `<title>-<user>` copy rather than
	the shared sidebar; `frappe.desk.sidebar_v16.add_user_specific_sidebar` promotes that
	copy over the shared one at boot.
	"""
...
def add_to_my_workspace(workspace):
	"""List a private workspace in its owner's "My Workspaces" sidebar.

	This is how private workspaces reach the sidebar under `Desktop Icon and Workspace
	Sidebar` navigation; workspace navigation surfaces them through the workspace selector
	instead. Deliberately *not* gated on the setting -- it is one child row per private
	workspace, and keeping it maintained on both means the sidebar is already correct if a
	site switches navigation later.
	"""
	try:
		...
		if existing_sidebar:
			...
			private_sidebar = frappe.copy_doc(frappe.get_doc("Workspace Sidebar", "My Workspaces"))
		...
		private_sidebar.save() if existing_sidebar else private_sidebar.insert()
