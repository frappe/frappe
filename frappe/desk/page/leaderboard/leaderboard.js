...
class Leaderboard {
	...
	get_date_range() {
		...
		let date_range_map = {
			"this week": [frappe.datetime.week_start(), frappe.datetime.now_date()],
			"this month": [frappe.datetime.month_start(), frappe.datetime.now_date()],
			"this quarter": [frappe.datetime.quarter_start(), frappe.datetime.now_date()],
			"this year": [frappe.datetime.year_start(), frappe.datetime.now_date()],
			"last week": [frappe.datetime.add_days(current_date, -7), frappe.datetime.now_date()],
			"last month": [frappe.datetime.add_months(current_date, -1), frappe.datetime.now_date()],
			"last quarter": [frappe.datetime.add_months(current_date, -3), frappe.datetime.now_date()],
			"last year": [frappe.datetime.add_months(current_date, -12), frappe.datetime.now_date()],
			"all time": null,
			"select date range": this.selected_date_range || [frappe.datetime.month_start(), frappe.datetime.now_date()]
		}
