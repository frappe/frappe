...
from frappe.core.api.document import add_share, get_shared_users, set_share_permission
...
class TestDocShare(IntegrationTestCase):
	...
	def test_add(self):
		...
		add_share("Event", self.event.name, self.user)
	...
	def test_doc_permission(self):
		...
		add_share("Event", second_event.name, self.user)
		with self.assertRowsRead(1):
			self.assertTrue(self.event.has_permission())
	...
	def test_list_permission(self):
		...
		doc.insert()
		add_share("Web Page", doc.name, self.user)
	...
	def test_share_permission(self):
		add_share("Event", self.event.name, self.user, write=1, share=1)
	...
	def test_set_permission(self):
		...
		frappe.set_user("Administrator")
		set_share_permission("Event", self.event.name, self.user, "share")
	...
	def test_permission_to_share(self):
		frappe.set_user(self.user)
		self.assertRaises(frappe.PermissionError, add_share, "Event", self.event.name, self.user)
		...
		# test not raises
		frappe.set_user(self.user)
		add_share("Event", self.event.name, "test1@example.com", write=1, share=1)
	...
	def test_share_with_everyone(self):
		...
		set_share_permission("Event", self.event.name, None, "read", everyone=1)
		self.assertTrue(self.event.name in frappe.share.get_shared("Event", self.user))
		...
		set_share_permission("Event", self.event.name, None, "read", value=0, everyone=1)
		self.assertTrue(self.event.name not in frappe.share.get_shared("Event", self.user))
	...
	def test_share_with_submit_perm(self):
		...
		frappe.set_user("Administrator")
		add_share(doctype, submittable_doc.name, self.user, submit=1)
	...
	def test_share_int_pk(self):
		...
		test_doc.insert()
		add_share("Console Log", test_doc.name, self.user)
	...
	def test_share_disabled_add(self):
		...
		add_share("Event", self.event.name, self.user, share=1)  # Share as admin
		frappe.set_user(self.user)

		# User does not have share access although given to them
		self.assertFalse(self.event.has_permission("share"))
		self.assertRaises(frappe.PermissionError, add_share, "Event", self.event.name, "test1@example.com")

	@IntegrationTestCase.change_settings("System Settings", {"disable_document_sharing": 1})
	def test_share_disabled_add_with_ignore_permissions(self):
		add_share("Event", self.event.name, self.user, share=1)
		frappe.set_user(self.user)
	...
	def test_cannot_share_without_permission(self):
		...
		try:
			...
			# Can share read
			add_share("Communication", doc.name, "test1@example.com")
		finally:
			doc.delete()
