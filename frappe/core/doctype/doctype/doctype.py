...
class DocType(Document):
        ...
        for fname in ('{}.js', '{}.py', '{}_list.js', '{}_calendar.js', 'test_{}.py', 'test_{}.js'):
            ...
            if os.path.exists(fname):
                ...
                with open(fname, 'w') as f:
                    ...
                    file_content = file_content.replace(frappe.scrub(old),
                                                        frappe.scrub(new))  # replace str with _ (py imports)
                    file_content = file_content.replace(old.replace(' ', ''),
                                                        new.replace(' ', ''))  # replace str (py controllers)
    ...
    def preserve_naming_series_options_in_property_setter(self):
        ...
        if not (frappe.db.table_exists(self.name) and frappe.db.sql(
    ...
    def make_controller_template(self):
        ...
        if self.has_web_view:
            templates_path = frappe.get_module_path(frappe.scrub(self.module), 'doctype', frappe.scrub(self.name),
                                                    'templates')
...
def validate_fields(meta):
    ...
    def check_unique_fieldname(docname, fieldname):
        ...
        if len(duplicates) > 1:
            frappe.throw(_("{0}: Fieldname {1} appears multiple times in rows {2}").format(docname, fieldname,
                                                                                           ", ".join(duplicates)),
    ...
    def check_illegal_mandatory(docname, d):
        if (d.fieldtype in no_value_fields) and d.fieldtype not in table_fields and d.reqd:
            frappe.throw(_("{0}: Field {1} of type {2} cannot be mandatory").format(docname, d.label, d.fieldtype),
    ...
    def check_link_table_options(docname, d):
        ...
        if d.fieldtype in ("Link",) + table_fields:
            if not d.options:
                frappe.throw(
                    _("{0}: Options required for Link or Table type field {1} in row {2}").format(docname, d.label,
                                                                                                  d.idx),
                    DoctypeLinkError)
            ...
            if d.options != d.parent:
                ...
                if not options:
                    frappe.throw(
                        _("{0}: Options must be a valid DocType for field {1} in row {2}").format(docname, d.label,
                                                                                                  d.idx),
    ...
    def check_hidden_and_mandatory(docname, d):
        if d.hidden and d.reqd and not d.default:
            frappe.throw(
                _("{0}: Field {1} in row {2} cannot be hidden and mandatory without default").format(docname, d.label,
                                                                                                     d.idx),
    ...
    def check_precision(d):
        if d.fieldtype in ("Currency", "Float", "Percent") and d.precision is not None and not (
    ...
    def check_unique_and_text(docname, d):
        ...
        if getattr(d, "unique", False):
            if d.fieldtype not in ("Data", "Link", "Read Only"):
                frappe.throw(_("{0}: Fieldtype {1} for {2} cannot be unique").format(docname, d.fieldtype, d.label),
                             NonUniqueError)
            ...
            if not d.get("__islocal") and frappe.db.has_column(d.parent, d.fieldname):
                ...
                if has_non_unique_values and has_non_unique_values[0][0]:
                    frappe.throw(
                        _("{0}: Field '{1}' cannot be set as Unique as it has non-unique values").format(docname,
                                                                                                         d.label),
                        NonUniqueError)

        if d.search_index and d.fieldtype in ("Text", "Long Text", "Small Text", "Code", "Text Editor"):
            frappe.throw(_("{0}:Fieldtype {1} for {2} cannot be indexed").format(docname, d.fieldtype, d.label),
                         CannotIndexedError)
        ...
        def _validate_title_field_pattern(pattern):
            ...
            for fieldname in re.findall("{(.*?)}", pattern, re.UNICODE):
                ...
                if fieldname not in fieldname_list:
                    frappe.throw(
                        _("{{{0}}} is not a valid fieldname pattern. It should be {{field_name}}.").format(fieldname),
