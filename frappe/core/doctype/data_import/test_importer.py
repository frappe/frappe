...
class TestTreeDataImport(IntegrationTestCase):
	...
	def test_tree_import(self):
		meta = frappe.get_meta(self.doctype_name)
		parent_field = meta.nsm_parent_field
		tree_doc_names = ("Root", "Division", "Leaf")

		# Delete children before parents for nested-set doctypes.
		self.addCleanup(_delete_doctype_records, self.doctype_name, tuple(reversed(tree_doc_names)))
		for name in reversed(tree_doc_names):
...
class TestTreeAliasDataImport(IntegrationTestCase):
	...
	def test_tree_alias_import(self):
		...
		self.addCleanup(self._cleanup_docs, labels)
