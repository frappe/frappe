...
from nts.core.doctype.file.exceptions import FileTypeNotAllowed
from nts.core.doctype.file.utils import get_corrupted_image_msg, get_extension
...
from nts.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	d = nts.new_doc("ToDo")
	d.description = "Test"
	d.assigned_by = nts.session.user
...
def make_test_image_file(private=False):
	file_path = nts.get_app_path("nts", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = nts.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: "File" = nts.get_doc("File", test_file.name)
...
class TestSimpleFile(ntsTestCase):
	def setUp(self):
	...
	def test_save(self):
		_file = nts.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
		self.assertEqual(content, self.test_content)


class TestFSRollbacks(ntsTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = nts.generate_hash()
		file = nts.new_doc("File", file_name=file_name, content=content).insert()
...
class TestSameFileName(ntsTestCase):
	def test_saved_content(self):
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
		...
		_file = nts.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = nts.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = nts.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = nts.get_doc("File", {"file_url": _file2.file_url})
		self.assertEqual(_file.get_content(), test_content2)


class TestSameContent(ntsTestCase):
	def setUp(self):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(nts.exceptions.AttachmentLimitReached, file2.insert)
		limit_property.delete()
		nts.clear_cache(doctype="ToDo")


class TestFile(ntsTestCase):
	def setUp(self):
	...
	def tearDown(self):
		try:
			nts.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except nts.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = nts.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = nts.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = nts.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = nts.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = nts.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			nts.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = nts.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(nts.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = nts.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = nts.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: "File" = nts.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://nts.io/files/nts.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: "File" = nts.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": nts.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = nts.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": nts.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = nts.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			nts.message_log[0].get("message"),
			f"File '{nts.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = nts.get_app_path("nts", "www/_test/assets/file.zip")
...
class TestAttachment(ntsTestCase):
	...
	def tearDownClass(cls):
		nts.db.rollback()
		nts.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = nts.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(ntsTestCase):
	def setUp(self) -> None:
		nts.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(ntsTestCase):
	def test_extract_images_from_doc(self):
		is_private = not nts.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(nts.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = nts.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{nts.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not nts.get_meta("ToDo").make_attachments_public
		test_doc = nts.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			nts.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not nts.get_meta("Communication").make_attachments_public
		...
		self.assertTrue(
			nts.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(ntsTestCase):
	...
	def test_optimize_svg(self):
		file_path = nts.get_app_path("nts", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = nts.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = nts.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(ntsTestCase):
	def setUp(self) -> None:
		nts.db.delete("File", {"is_folder": 0})
		nts.get_doc(
			doctype="DocType",
	...
	def tearDown(self) -> None:
		nts.set_user("Administrator")
		nts.db.rollback()
		nts.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = nts.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + nts.generate_hash()
		content = file_name.encode()

		doc_pub: "File" = nts.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: "File" = nts.new_doc("File")  # type: ignore
