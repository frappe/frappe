...
frappe.listview_settings['File'] = {
	...
	prepare_data: function(data) {
		...
		if(data.is_folder) {
			icon += '<i class="fa fa-folder-close-alt icon-fixed-width"></i> ';
		} else if(frappe.utils.is_image_file(data.file_name)) {
			icon += '<i class="fa fa-picture icon-fixed-width"></i> ';
		} else {
			icon += '<i class="fa fa-file-alt icon-fixed-width"></i> '
		}

		data._title = icon + (data.file_name ? data.file_name : data.file_url)

		if (data.is_private) {
			data._title += ' <i class="fa fa-lock icon-fixed-width text-warning"></i>'
		}
	},
