frappe.ui.form.on("Communication", {
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "frappe.email.api.get_communication_doctype" };
					},
				},
				{
			...
			if (values) {
				...
				frappe.confirm(
					...
					function () {
						d.hide();
						frappe.call({
							method: "frappe.email.api.relink_communication",
							args: {
								name: frm.doc.name,
	...
	show_move_dialog: function (frm) {
		var d = new frappe.ui.Dialog({
			...
			primary_action(values) {
				return frappe.call({
					method: "frappe.email.api.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return frappe.call({
			method: "frappe.email.api.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return frappe.call({
			method: "frappe.email.api.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	mark_as_spam: function (frm) {
		frappe.call({
			method: "frappe.email.api.mark_as_spam",
			args: {
				communication: frm.doc.name,
	...
	move_to_trash: function (frm) {
		frappe.call({
			method: "frappe.email.api.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
