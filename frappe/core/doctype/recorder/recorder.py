...
import nts
from nts import _
from nts.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from nts.custom.doctype.property_setter.property_setter import make_property_setter
from nts.model.document import Document
from nts.recorder import RECORDER_REQUEST_HASH
from nts.recorder import get as get_recorder_data
from nts.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from nts.core.doctype.recorder_query.recorder_query import RecorderQuery
		from nts.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = nts._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = nts.as_json(i["stack"])
			i["explain_result"] = nts.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=nts.as_json(request.get("headers"), indent=4),
		form_dict=nts.as_json(request.get("form_dict"), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		nts.enqueue(_add_index, table=index["table"], column=index["column"])
	nts.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	nts.db.add_index(doctype, [column])
	nts.msgprint(
...
def optimize(recorder_id: str):
	nts.only_for("Administrator")
	nts.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = nts.get_doc("Recorder", recorder_id)
	...
	data = nts.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	nts.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	nts.publish_realtime("recorder-analysis-complete", user=nts.session.user)
	nts.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_nts_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not nts.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in nts.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in nts.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return nts.db.sql(f"select count(distinct {column}) from {table}")[0][0]
