...
@frappe.whitelist()
def invite_by_email(emails: str, role: str, redirect_to_path: str, app_name: str = FRAMEWORK_APP_NAME) -> dict[str, list[str]]:
    ...
    if not is_app_framework:
        ...
        try:
            ...
            if (
             not isinstance(user_invitation_hook, dict) or
             not isinstance(user_invitation_hook.get(allowed_invite_roles_key), list) or
             not all(isinstance(r, str) for r in user_invitation_hook.get(allowed_invite_roles_key))
            ):
    ...
    existing_user_emails = frappe.db.get_all(
        "User", 
        filters = {"email": ["in", email_list]}, 
        pluck = "email"
    )
    existing_invited_emails_filters = {
        "email": ["in", email_list]
    }
    if not is_app_framework:
    ...
    existing_invited_emails = frappe.db.get_all(
        ...
        filters = existing_invited_emails_filters,
        pluck = "email"
    )
...
def accept_invitation(key: str) -> None:
    ...
    if invitation.status == "Pending":
        ...
        try:
            ...
            if (
                isinstance(user_invitation_hook, dict) and
                isinstance(user_invitation_hook.get(after_accept_key), list)
            ):
    ...
    should_update_password = not user.last_password_reset_date and not bool(frappe.get_system_settings("disable_user_pass_login"))
