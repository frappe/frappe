...
from nts.utils import cstr, random_string
...
class CustomField(Document):
		...
		if not self.fieldname:
			...
			if not label:
				if self.fieldtype in ["Section Break", "Column Break", "Tab Break"]:
					...
					nts.throw(_("Label is mandatory"))
	...
	def validate(self):
		...
		from nts.core.doctype.doctype.doctype import check_fieldname_conflicts
		...
		if not self.fieldname:
			nts.throw(_("Fieldname not set for Custom Field"))
	...
	def on_update(self):
		...
		if not nts.flags.in_create_custom_fields:
			nts.clear_cache(doctype=self.dt)
			nts.db.updatedb(self.dt)

	def on_trash(self):
		# check if Admin owned field
		if self.owner == "Administrator" and nts.session.user != "Administrator":
			nts.throw(
				_(
					"Custom Field {0} is created by the Administrator and can only be deleted through the Administrator account."
				).format(nts.bold(self.label))
			)

		# delete property setter entries
		nts.db.delete("Property Setter", {"doc_type": self.dt, "field_name": self.fieldname})

		# update doctype layouts
		doctype_layouts = nts.get_all("DocType Layout", filters={"document_type": self.dt}, pluck="name")
	...
	def validate_insert_after(self, meta):
		if not meta.get_field(self.insert_after):
			nts.throw(
				...
				nts.DoesNotExistError,
			)

		if self.fieldname == self.insert_after:
			nts.throw(_("Insert After cannot be set as {0}").format(meta.get_label(self.insert_after)))
...
def get_fields_label(doctype=None):
	...
	if doctype in core_doctypes_list:
		return nts.msgprint(_("Custom Fields cannot be added to core DocTypes."))

	if meta.custom:
		return nts.msgprint(_("Custom Fields can only be added to a standard DocType."))

	return [
		{"value": df.fieldname or "", "label": _(df.label, context=df.parent) if df.label else ""}
		for df in nts.get_meta(doctype).get("fields")
	]


def create_custom_field_if_values_exist(doctype, df):
	df = nts._dict(df)
	if df.fieldname in nts.db.get_table_columns(doctype) and nts.db.count(
...
def create_custom_field(doctype, df, ignore_validate=False, is_system_generated=True):
	...
	if not df.fieldname and df.label:
		df.fieldname = nts.scrub(df.label)
	if not nts.db.get_value("Custom Field", {"dt": doctype, "fieldname": df.fieldname}):
...
def create_custom_fields(custom_fields: dict, ignore_validate=False, update=True):
	...
	try:
		nts.flags.in_create_custom_fields = True
		doctypes_to_update = set()

		if nts.flags.in_setup_wizard:
			...
			for doctype in doctypes:
				...
				for df in fields:
					...
					if not field:
						...
						except nts.exceptions.DuplicateEntryError:
							pass

					elif update:
						custom_field = nts.get_doc({"doctype": "Custom Field", **field})
		...
		nts.flags.in_create_custom_fields = False
...
def get_existing_custom_fields(custom_fields):
	...
	existing_fields = nts.get_all("Custom Field", filters={"dt": ("in", doctypes_to_fetch)}, fields="*")
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	field: CustomField = nts.get_doc("Custom Field", custom_field)
	...
	if field.is_system_generated:
		nts.throw(_("System Generated Fields can not be renamed"))
	if nts.db.has_column(parent_doctype, fieldname):
		nts.throw(_("Can not rename as column {0} is already present on DocType.").format(fieldname))
	if old_fieldname == new_fieldname:
		nts.msgprint(_("Old and new fieldnames are same."), alert=True)
		return

	if nts.db.has_column(field.dt, old_fieldname):
		nts.db.rename_column(parent_doctype, old_fieldname, new_fieldname)
	...
	nts.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)
	nts.db.commit()
	nts.clear_cache()
