...
class CustomServerAction(Document):
	...
	def validate_python_code(self, doctype, field_name, field_to_validate):
		try:
			temp_doc = frappe.get_last_doc(self.document_type)
		...
		if field_to_validate:
			try:
				if field_name == 'Code':
					result = test_python_expr(field_to_validate, mode='exec')
				else:
					result = safe_eval(field_to_validate, None, get_context(temp_doc))
