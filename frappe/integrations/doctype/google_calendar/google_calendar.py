		...
		try:
			...
			if err.resp.status == 410:
				set_encrypted_password(
					"Google Calendar", account.name, "", "next_sync_token"
				)
				frappe.db.commit()
				msg += " " + _(
					"Sync token was invalid and has been resetted, Retry syncing."
				)
	...
	for idx, event in enumerate(results):
		...
		if event.get("status") == "confirmed":
			...
			if not frappe.db.exists(
				"Event", {"google_calendar_event_id": event.get("id")}
			):
...
def insert_event_in_google_calendar(doc, method=None):
	...
	try:
		event = (
			google_calendar.events()
			.insert(calendarId=doc.google_calendar_id, body=event)
			.execute()
		)
		frappe.db.set_value(
			"Event",
...
def update_event_in_google_calendar(doc, method=None):
	...
	try:
		event = (
			google_calendar.events()
			.get(
				calendarId=doc.google_calendar_id, eventId=doc.google_calendar_event_id
			)
			.execute()
		)
...
def repeat_on_to_google_calendar_recurrence_rule(doc):
	...
	if doc.repeat_on == "Weekly":
		byday = [
			framework_days.get(day.lower()) for day in weekdays if doc.get(day.lower())
		]
