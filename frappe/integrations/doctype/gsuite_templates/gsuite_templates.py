...
@frappe.whitelist()
def create_gsuite_doc(doctype, name, template=None):
	templ = frappe.get_doc('GSuite Templates', template)
	...
	filedata = save_url(r['url'], filename, doctype, name, "Home/Attachments", True)
	comment = frappe.get_doc(doctype, name).add_comment("Attachment",
