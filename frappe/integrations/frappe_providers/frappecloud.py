...
def get_remote_script(remote_site):
    ...
    if request.status_code / 100 != 2:
        print(
            f"Request exited with Status Code: {request.status_code}\nPayload: {html2text(request.text)}"
        )
        click.secho(
...
def frappecloud_migrator(local_site,remote_site_name=None):
    if remote_site_name == None:
        remote_site_name = frappe.conf.frappecloud_url or "frappecloud.com"
    ...
    os.execv(py, [py, script.name, local_site,remote_site_name])
