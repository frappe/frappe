...
def claim_batch(batch_size=DEFAULT_BATCH_SIZE) -> list[str]:
	...
	# The only interpolations are a module constant and a fixed lock clause; every value is a
	# bound parameter.
	# nosemgrep
	rows = frappe.db.sql(
		f"""
	...
	# The claim has to be visible to other drainers immediately, and the row locks taken above
	# must be released before the (slower) execution phase.
	frappe.db.commit()  # nosemgrep
	return names
