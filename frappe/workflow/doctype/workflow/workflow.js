nts.provide("nts.core");

nts.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		let message = `
		<div class="flex">
			<div class="mr-3"><img style="border-radius: var(--border-radius-md)" width="600" src="/assets/nts/images/workflow-builder.gif"></div>
			<div>
				...
				<div>
					<a class="btn btn-primary btn-sm" href="${workflow_builder_url}">
						${__("Workflow Builder")} ${nts.utils.icon("right", "xs")}
					</a>
				</div>
			</div>
		</div>
	...
	update_field_options: function (frm) {
		...
		nts.model.with_doctype(doc.document_type, () => {
			const fieldnames = nts
				.get_meta(doc.document_type)
				.fields.filter((field) => !nts.model.no_value_type.includes(field.fieldtype))
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return nts.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return nts
			.xcall("nts.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					nts.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

nts.ui.form.on("Workflow Document State", {
...
nts.ui.form.on("Workflow Transition", {
	state: function (_, cdt, cdn) {
		var row = locals[cdt][cdn];
		delete row.workflow_builder_id;
	},
