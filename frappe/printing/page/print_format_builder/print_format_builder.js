nts.pages["print-format-builder"].on_page_load = function (wrapper) {
	nts.print_format_builder = new nts.PrintFormatBuilder(wrapper);
	nts.breadcrumbs.add("Setup", "Print Format");
};

nts.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = nts.get_route();
	if (route.length > 1) {
		nts.model.with_doc("Print Format", route[1], function () {
			nts.print_format_builder.print_format = nts.get_doc("Print Format", route[1]);
			nts.print_format_builder.refresh();
		});
	} else if (nts.route_options) {
		if (nts.route_options.make_new) {
			let { doctype, name, based_on, beta } = nts.route_options;
			nts.route_options = null;
			nts.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			nts.print_format_builder.print_format = nts.route_options.doc;
			nts.route_options = null;
			nts.print_format_builder.refresh();
		}
	}
...
nts.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = nts.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main nts-card"></div>'
	...
	show_start() {
		this.page.main.html(nts.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = nts.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			nts.model.with_doc("Print Format", name, function (doc) {
				nts.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = nts.ui.form.make_control({
		...
		this.name_input = nts.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				nts.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		nts.call({
			method: "nts.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		nts.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = nts.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				nts.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(nts.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			nts.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						nts.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!nts.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(nts.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = nts.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = nts.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				nts.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = nts.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				nts.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		nts.call({
			...
			callback: function (r) {
				...
				nts.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
