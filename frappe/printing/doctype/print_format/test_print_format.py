...
class TestPrintFormatFor(IntegrationTestCase):
	"""Formats created before `print_format_for` existed carry NULL."""

	FORMAT_NAME = "_Test Legacy Print Format For"
	...
	def picker_options(self):
		return frappe.get_all(
			"Print Format",
			filters={"doc_type": "ToDo", "print_format_for": ["in", ["DocType", ""]]},
			pluck="name",
		)

	def test_print_page_picker_lists_legacy_formats(self):
		self.assertIn(self.FORMAT_NAME, self.picker_options())

	def test_patch_backfills_and_is_idempotent(self):
		from frappe.patches.v16_0.set_print_format_for_doctype import execute

		execute()
		self.assertEqual(frappe.db.get_value("Print Format", self.FORMAT_NAME, "print_format_for"), "DocType")
