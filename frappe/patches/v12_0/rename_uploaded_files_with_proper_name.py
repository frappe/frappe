...
def execute():
	...
	for f in frappe.get_all("File", filters={"is_folder": 0, "file_name": ["like", "%/%"]}, fields=['file_url', 'name', 'file_name']):
		filename = f.file_name.rsplit('/', 1)[-1]

		set_query = 'file_name = {0}'.format(filename)

		if not f.file_url:
			fileurl = f.file_name
			set_query += ', file_url = {0}'.format(fileurl)

		frappe.db.sql('''
		update `tabFile` set {0} where name = {1}
		'''.format(set_query, f.name))
