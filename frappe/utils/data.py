...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = frappe.db.get_value("Marketing Source", source, "slug") or source
		_campaign = frappe.db.get_value("Marketing Campaign", campaign, "slug") or campaign
		_medium = frappe.db.get_value("Marketing Medium", medium, "slug") or medium
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = frappe.db.get_value("Marketing Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not frappe.db.exists("Marketing Source", source):
			source = frappe.new_doc("Marketing Source")
	...
	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = frappe.db.get_value("Marketing Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not frappe.db.exists("Marketing Medium", medium):
			medium = frappe.new_doc("Marketing Medium")
	...
	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = (
			frappe.db.get_value("Marketing Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		)
		if create and campaign == url_campaign and not frappe.db.exists("Marketing Campaign", campaign):
