...
def get_queues_timeout():
	"""
	Get the timeout values for different queues.

	This function retrieves a dictionary containing timeout values for different
	queues. It first gets the common site configuration using the
	'frappe.get_conf()' function. Then, it gets the custom workers
	configuration from the common site configuration. It sets a default
	timeout value of 300. The function returns a dictionary with default
	timeout values for 'short' and 'default' queues, a timeout value of 1500
	for the 'long' queue, and any custom timeout values specified in the
...
def enqueue(
 ...
 :param event: this is passed to enable clearing of jobs from queues
	    :param is_async: if is_async=False, the method is executed immediately,
 else via a worker
	    :param job_name: [DEPRECATED] can be used to name an enqueue call, which
 can be used to prevent duplicate calls
...
def _freeze_gc():
	"""Freeze the garbage collector if frappe._tune_gc is True."""
	if frappe._tune_gc:
	    gc.collect()
	    gc.freeze()
...
def get_workers(queue=None):
	"""
	Returns a list of Worker objects tied to a queue object if queue is passed,
	else returns a list of all workers.

	Args:
	    queue (Queue, optional): The queue object to filter workers by.

	Returns:
	    list: A list of Worker objects.
	"""
	if queue:
...
def get_running_jobs_in_queue(queue):
	"""
	Returns a list of Jobs objects that are tied to a queue object and are
	currently running.

	Args:
	    queue (Queue): The queue object to filter running jobs by.
...
def get_queue(qtype, is_async=True):
	...
	Args:
	    qtype (str): The type of the queue.
	    is_async (bool, optional): Whether the queue is asynchronous.
...
def validate_queue(queue, default_queue_list=None):
	"""
	Validates if a given queue is valid.

	Args:
	    queue (Queue): The queue to validate.
	    default_queue_list (list, optional): The list of default queues to compare against.
	        Defaults to None.

	Raises:
	    Exception: If the queue is not in the list of default queues.
	"""
...
def get_redis_connection_without_auth():
	"""Get a Redis connection without authentication.
...
def get_queues(connection=None) -> list[Queue]:
	...
	Returns:
	    list[Queue]: The list of queues linked to the current bench.
	"""
...
def generate_qname(qtype: str) -> str:
	"""Generate a queue name by combining the bench ID and queue type.

	Args:
	    qtype (str or List[str]): The type(s) of the queue.
...
def is_queue_accessible(qobj: Queue) -> bool:
	"""Check if a queue is related to the current bench.

	Args:
	    qobj (Queue): The queue object.

	Returns:
	    bool: True if the queue is related to the current bench, False otherwise.
	"""
...
def test_job(s):
	"""A test job that sleeps for a specified amount of time.

	Args:
	    s (int): The amount of time to sleep.
	"""
	import time
...
def is_job_enqueued(job_id: str) -> bool:
	"""Check if a job is enqueued.

	Args:
	    job_id (str): The job ID.

	Returns:
	    bool: True if the job is enqueued, False otherwise.
	"""
...
def get_job(job_id: str) -> Job:
	"""Fetches the job object with the given job ID."""
	try:
