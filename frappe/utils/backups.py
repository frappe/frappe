...
class BackupGenerator:
	...
	def take_dump(self):
		...
		if not gzip_exc:
			frappe.throw(
				f"gzip not found in PATH! This is required to take a backup.", exc=frappe.ExecutableNotFound
			)
		...
		if self.db_type == "mariadb":
			if self.backup_includes:
				...
				extra.extend([f"--ignore-table='{frappe.conf.db_name}.{table}'" for table in self.backup_excludes])
		...
		if not bin:
			frappe.throw(
				f"{bin} not found in PATH! This is required to take a backup.", exc=frappe.ExecutableNotFound
			)
		cmd.append(bin)
		cmd.extend(args)
