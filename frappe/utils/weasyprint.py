...
def download_pdf(doctype, name, print_format, letterhead=None):
    """
    Generate a PDF file from the specified document, print format, and letterhead.

    This function generates a PDF file from the given document, print format, and
    letterhead.
    The generated PDF is then sent as a response.

    Args:
        doctype (str): The type of the document.
        name (str): The name of the document.
        print_format (str): The print format to be used.
        letterhead (str, optional): The letterhead to be used. Defaults to None.
    """
...
def get_html(doctype, name, print_format, letterhead=None):
    """
    Generate an HTML preview of the specified document, print format, and letterhead.

    This function generates an HTML preview of the given document using the specified
    print format and letterhead. The HTML preview is then returned.
    ...
    Returns:
        str: The generated HTML preview.
    """
    doc = frappe.get_doc(doctype, name)
    doc.check_permission("print")
    generator = PrintFormatGenerator(print_format, doc, letterhead)
...
class PrintFormatGenerator:
    """
    Generate a PDF of a Document, with repeatable header and footer if letterhead
    is provided.
    ...
    def build_context(self):
        """
        Initialize the context dictionary with the necessary values.
        """
    ...
    def get_html_preview(self):
        """
        Get the HTML preview of the document.
        """
    ...
    def get_main_html(self):
        """
        Get the main HTML of the document.
        """
    ...
    def get_layout(self, print_format):
        """
        Get the layout for the specified print format.

        This function takes a print_format parameter and returns the layout for the
        specified print format after performing necessary operations on it.

        Args:
            print_format (str): The print format to get the layout for.

        Returns:
            dict: The layout for the specified print format.
        """
    ...
    def set_field_renderers(self, layout):
        """
        Set field renderers for each field in the layout.

        This function sets field renderers for each field in the provided layout using
        a predefined dictionary.

        Args:
            layout (dict): The layout to set field renderers for.

        Returns:
            dict: The modified layout with field renderers set.
        """
    ...
    def process_margin_texts(self, layout):
        """
        Process margin texts in the layout.

        This function processes margin texts in the provided layout by rendering
        template variables using the frappe.render_template function.

        Args:
            layout (dict): The layout to process margin texts for.

        Returns:
            dict: The modified layout with processed margin texts.
        """
        margin_texts = [
        	"top_left",
        	"top_center",
...
def import_weasyprint():
    """
    Import the CSS and HTML classes from the weasyprint module.

    This function attempts to import the CSS and HTML classes from the weasyprint module.
    If the import is successful, it returns the imported classes.
    If an OSError is raised during the import, it catches the exception
    and throws a message with instructions for installing the necessary system
    dependencies.

    Returns:
        tuple: A tuple containing the imported HTML and CSS classes.

    Raises:
        frappe.ValidationError: If the import fails due to missing system dependencies.
    """
    try:
