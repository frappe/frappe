...
class BotParser(object):
	...
	def format_list(self, data):
		...
		return _("I found these: ") + ", ".join(
			[
				" [{title}](/app/Form/{doctype}/{name})".format(
					title=d.title or d.name, doctype=self.get_doctype(), name=d.name
				)
				for d in data
			]
		)
		...
		if self.has("whatsup", "what's up", "wassup", "whats up", "notifications", "open tasks"):
			...
			if open_items:
				return "Following items need your attention:\n\n" + "\n\n".join(
					["{0} [{1}](/app/List/{1})".format(d[1], d[0]) for d in open_items if d[1] > 0]
				)
			else:
		...
		if self.startswith("open", "show open", "list open", "get open"):
			if self.tables:
				...
				if filters:
					...
					return ", ".join(
						"[{title}](/app/Form/{doctype}/{name})".format(
							doctype=doctype, name=d.get("name"), title=d.get("title") or d.get("name")
						)
						for d in data
					)
				else:
	...
	def get_reply(self):
		...
		if self.startswith(_("list"), _("show")):
			...
			if self.tables:
				...
				filters = {}
				if like:
					filters = {meta.title_field or "name": ("like", "%" + like + "%")}
		...
		if self.startswith("find", "search"):
			...
			if table:
				...
				if self.tables:
					...
					data = frappe.get_list(self.get_doctype(), filters=filters, or_filters=or_filters)
					if data:
