...
def validate_pdf_file(file_path):
    ...
    if not filename.lower().endswith('.pdf'):
        frappe.throw(_("Only PDF files are allowed for EmbedPdf field type."))
    
    # Check MIME type if possible
    mime_type, _ = mimetypes.guess_type(file_path)
    if mime_type and mime_type != 'application/pdf':
        frappe.throw(_("File must be of type PDF (application/pdf)."))
...
def validate_embed_pdf_field(doctype, fieldname, file_url):
    ...
    try:
        ...
        if not file_url.lower().endswith('.pdf'):
            return {
                "valid": False,
                "message": _("Only PDF files are allowed for EmbedPdf field type.")
            }
        ...
        frappe.log_error(f"EmbedPdf validation error for Doctype: {doctype}, Field: {fieldname}, Error: {str(e)}")
        return {
            "valid": False,
            "message": _("Error validating PDF file.")
        }