...
def get_print_context():
	"""The render `get_print` / `render_pdf` set up before calling `pdf_generator` hooks."""
	return getattr(frappe.local, "print_context", None)


def set_print_context(**values):
	previous = get_print_context()
	frappe.local.print_context = frappe._dict(values)
	return previous


def restore_print_context(previous):
	if previous is None:
		frappe.local.print_context = None
	else:
		frappe.local.print_context = previous


def generator_from_print_context(print_format) -> "PrintFormatGenerator | None":
	"""Generator for a `pdf_generator` hook: the one already rendering, else one built
	from the print context, else (legacy callers) from the request's form_dict."""
	from frappe.model.document import Document

	ctx = get_print_context()
	if ctx is None:
		ctx = frappe._dict(frappe.form_dict)
		ctx.pop("generator", None)
	if ctx.get("generator"):
	...
	return PrintFormatGenerator(pf, doc, ctx.get("letterhead"), no_letterhead=ctx.get("no_letterhead"))
...
def classic_layout_message():
	return _("This format still has a classic layout. Convert it to the new builder first.")


def zone_from_html(html) -> dict:
	"""Wrap a legacy header/footer HTML string in a zone, the way the builder's
	`migrate_to_section` does on open."""
	fields = []
	if isinstance(html, str) and html.strip():
		fields.append({"fieldtype": "HTML", "fieldname": "_zone_html", "label": "", "html": html})
	return {"columns": [{"label": "", "fields": fields}]}
	...
	def render_pdf(self, password=None):
		...
		if generator_name not in ("chrome", "wkhtmltopdf"):
			previous = set_print_context(
				...
				name=self.doc.name,
				doc=self.doc,
				letterhead=self.letterhead and self.letterhead.name,
				no_letterhead=not self.letterhead,
				generator=self,
			)
			try:
