...
class TestPrintFormatGenerator(IntegrationTestCase):
	...
	def _make_merged_table_setup(self):
		...
		self.addCleanup(child.delete, ignore_permissions=True, force=True)

		parent = new_doctype(
			fields=[
				{"label": "Items", "fieldname": "items", "fieldtype": "Table", "options": child.name},
			],
		).insert(ignore_permissions=True)
		self.addCleanup(parent.delete, ignore_permissions=True, force=True)

		doc = frappe.get_doc(
			{
				"doctype": parent.name,
				"items": [{"item_name": "2 < 3 widget", "description": "<p>Rich one</p><p>Rich two</p>"}],
			}
		).insert(ignore_permissions=True)
		self.addCleanup(doc.delete, ignore_permissions=True)
		...
		pf = frappe.get_doc(
			{
				"doctype": "Print Format",
				"name": f"_Test Merged RT {frappe.generate_hash(length=6)}",
		...
		self.addCleanup(pf.delete, ignore_permissions=True)
		return parent, doc, pf

	def test_merged_rich_text_renders_html_plain_stays_escaped(self):
		"""A Text Editor field merged into a table column keeps its HTML; a plain
		merged field stays escaped. Covers both the print output and the builder
		preview, which share macros/Table.html."""
		from frappe.utils.print_format_generator import get_html, render_builder_preview

		parent, doc, pf = self._make_merged_table_setup()

		for label, html in (
			("print", get_html(parent.name, doc.name, pf.name, no_letterhead=1)),
			(
				"preview",
				render_builder_preview(pf.as_dict(), parent.name, doc.name, letterhead="No Letterhead"),
			),
		):
			with self.subTest(render=label):
				self.assertIn("<p>Rich one</p>", html)
				self.assertIn("2 &lt; 3 widget", html)
