...
class TestPrintFormatGenerator(IntegrationTestCase):
	...
	def test_pdf_generator_resolution_keeps_the_stored_choice(self):
		"""A plain format prints with its own engine (wkhtmltopdf by default), while a
		beta format stays pinned to chrome — wkhtmltopdf cannot lay out its flexbox columns."""
		...
		self.assertEqual(resolve_pdf_generator(beta, "wkhtmltopdf"), "chrome")
	...
	def test_download_pdf_without_format_uses_the_doctype_default(self):
		...
		self.assertIsNone(jinja.call_args.kwargs["format"])
	...
	def _beta_format_for(self, doctype):
		pf = frappe.get_doc(
			{
				"doctype": "Print Format",
				"name": f"_Test PFG Status {frappe.generate_hash(length=6)}",
				"doc_type": doctype,
				"print_format_builder_beta": 1,
				"custom_format": 0,
				"standard": "No",
	...
	def _render_standard(self, doc):
		...
		return get_html_and_style(doc=doc.as_json(), print_format=pf.name, no_letterhead=1)["html"] or ""
