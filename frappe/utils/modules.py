...
def get_modules_from_all_apps_for_user(user: str | None = None) -> list[dict]:
	...
	global_blocked_modules = User.docs.get("Administrator", cached=True).get_blocked_modules()
	user_blocked_modules = User.docs.get(user, cached=True).get_blocked_modules()
