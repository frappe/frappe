...
def get_change_log(user=None):
	"""
	Retrieve the change log for a user.

	If no user is specified, the change log for the current user is retrieved.
	The change log includes the titles, descriptions, versions, and detailed change
	log for each app.

	Args:
		user (str, optional): The user for whom to retrieve the change log. Defaults to None.

	Returns:
		list: The change log as a list of dictionaries.
	"""
	if not user:
...
def parse_latest_non_beta_release(response):
	"""
	Parses the response JSON for all the releases and returns the latest non-beta release

	Parameters
	----------
	response : list
		Response object returned by GitHub

	Returns
	-------
	json
		JSON object pertaining to the latest non-beta release
	"""
	version_list = [
...
def add_message_to_redis(update_json):
	"""
	Add a JSON message to the Redis cache.

	This function sets the value of the key 'update-info' in the Redis cache to the
	JSON representation of the `update_json` parameter. It then retrieves a list of
	enabled users from the 'User' table using the `frappe.get_all` function
	and filters the list to only include users with the role 'System Manager'.
	Finally, it adds the system managers to the Redis cache under the key
	'update-user-set'.

	Args:
		update_json (dict): The JSON message to be added to the Redis cache.
	"""
