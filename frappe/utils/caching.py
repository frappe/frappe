...
def __generate_request_cache_key(args: tuple, kwargs: dict):
	...
	This function generates a key for the cache based on the input arguments and
	keyword arguments.

	Args:
	    args (tuple): The input arguments for the function.
	    kwargs (dict): The keyword arguments for the function.

	Returns:
	    int: The generated cache key.
	"""
	if not kwargs:
	    return hash(args)
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None) -> Callable:
	...
	Args:
	    ttl (int | None): Time to expiry in seconds, defaults to 1 hour.
	    user (str | bool | None): Flag indicating whether the cache should be
	        specific to the session user.
	"""
