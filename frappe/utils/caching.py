...
def redis_cache(
	ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False, maxsize: int = 8
) -> Callable:
	"""Decorator to cache method calls and its return values in Redis
		# TODO: implement some policy to evict, but since `redis_cache` is used very sparingly this adds unexpected complexity and we avoid that for now.
		# Just set `maxsize` to be double or higher count for desired functions for now.
	args:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			# a `boot-strapping` problem, as internally `redis_wrapper` depends on `local.conf`. We also wait for `frappe.cache` to be available.
			if not (frappe.cache) or not (hasattr(frappe.local, "conf")):
				return func(*args, **kwargs)
