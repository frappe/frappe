...
def safe_b64decode(binary: bytes) -> bytes:
 ...
 Refs:
	     * https://en.wikipedia.org/wiki/Base64
	            * https://stackoverflow.com/questions/2941995/python-ignore-incorrect-
	     padding-error-when-base64-decoding
 """
	return base64.b64decode(binary + b"===")


def get_file_url(file_data_name):
	"""Get the URL of a file stored in the database.

	This function retrieves the file name and file URL from the database
	for the given file data name and returns the file URL if it exists,
	otherwise it returns the file name.

	Args:
	    file_data_name (str): The name of the file data in the database.

	Returns:
	    str: The URL of the file, or the file name if URL does not exist.
	"""
...
def upload():
	"""
	Handles uploading a file and adding a comment to a document.

	Returns:
	    dict: A dictionary containing the file information and the comment as a dictionary.
	"""
	# get record details
...
def save_uploaded(dt, dn, folder, is_private, df=None):
	"""Saves an uploaded file to a file document and returns the file document."""
...
def get_file_data_from_hash(content_hash, is_private=0):
	"""
	Retrieve file data from the database based on the content hash and privacy level.

	This function returns a dictionary of file attributes if the file exists, or
	False if it does not.

	Args:
	    content_hash (str): The content hash of the file.
	    is_private (int, optional): The privacy level of the file.

	Returns:
	    dict: A dictionary of file attributes if the file exists, or False if it does not.
	"""
...
def save_file_on_filesystem(fname, content, content_type=None, is_private=0):
	"""
	Write the file content to the file system and return the file name and URL.
	...
	Returns:
	    dict: A dictionary containing the file name and URL.
	"""
...
def get_max_file_size():
	"""
	Retrieve the maximum file size from the configuration settings.

	Returns:
	    int: The maximum file size.
	"""
...
def check_max_file_size(content):
	"""
	Check if the file size exceeds the maximum allowed size and raise an exception
	if it does.
...
def delete_file_from_filesystem(doc, only_thumbnail=False):
	...
	Args:
	    doc: The File document only_thumbnail (bool, optional): Delete only
	        the thumbnail if True. Defaults to False.
	"""
	if only_thumbnail:
...
def get_file(fname):
	...
	Returns:
	    list: A list containing the file name and content
	"""
...
def get_file_name(fname, optional_suffix):
	"""Returns a modified file name based on the given file name and optional suffix"""
	# convert to unicode
	fname = cstr(fname)
...
def is_safe_path(path: str) -> bool:
	...
	This function checks if the path starts with 'http://' or 'https://' and
	returns True in that case. Otherwise, it gets the absolute paths of the
	'path' and the current site path using the 'os.path' module and compares
	them. If they are the same, it returns True, otherwise False.
