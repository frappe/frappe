...
def remove_duplicate_fields(doctype):
	for field in frappe.db.sql(
		"""select fieldname, count(1) as cnt from tabDocField where parent=%s group by fieldname having cnt > 1""",
		doctype,
	):
		frappe.db.sql(
			"delete from tabDocField where fieldname=%s and parent=%s limit 1", (field[0], doctype)
		)
...
def set_property_setter(doctype, local_doc, original_doc):
	...
	for docfield in original_fields:
		for prop in original_fields[docfield]:
			...
			if docfield not in local_fields:
				continue

			if (
				prop in docfield_properties
				and prop in local_fields[docfield]
				and original_fields[docfield][prop] != local_fields[docfield][prop]
			):
				# make property_setter equivalent of local changes
				make_property_setter(
					doctype, docfield, prop, local_fields[docfield][prop], docfield_properties[prop]
				)
