...
rights_map = {"read": '11', "print": '12', "email": '13', "export": '14', "report": '15', "share": '16',
			"create": '21', "write": '22', "delete": '23', "amend": '24', "cancel": '25', "import": '26',
			"submit": '27'}
...
def validate_auth_field(doctype, auth_obj):
	"""filter out/ make sure authorization object with which all auth fields are valid doctype fields can be assigned
	to the target doctype,  """
...
def get_user_authorizations(user=None):
	...
	user_authorizations = frappe.cache().hget('user:' + user, 'get_user_authorizations',
											  lambda: _get_user_authorizations(user))
...
def get_authorizations(doctype, act='11', user=None, usage='doc', auth_obj=None):
	...
	return frappe.cache().hget('user:' + user, auth_key,
				lambda: _get_authorizations(doctype, act, user, usage, auth_obj=auth_obj))
...
def _auth_check(doctype=None, act='read', doc=None, user=None, auth_obj=None, verbose=None):
	...
	if not doc and not auth_obj:  # doctype level check
		...
		if not mandatory_auths:
			...
			save_check_log(check_log, user, mandatory_objs)
			return False
		return True
	else:
...
def save_check_log(check_log, user, auth_obj_recs=None, doc=None):
	...
	if auths:
		...
		fieldname = ['authorization_id', 'authorization_object', 'authorization_field', 'value_from', 'value_to',
					 'role']
		recs = []
	...
	frappe.local.rollback_observers.append(auth_check_log)  # to resume after rollback at end transaction
...
def auth_check_doc_fields(doc, user=None):
	"""reset doc field to none if not authorized, called by apply field level authorization"""
	if not user: user = frappe.session.user
	if user == "Administrator" or frappe.flags.in_install:
		return True
...
def build_condition(auth, table_alias, doctype, user):
...
def get_match_conditions(doctype, act='read', user=None, tables=None, parent_doctype=None, verbose=None):
	""" apply auth relevant restriction to db_query's where condition, called by via get_list/get_all or report query
	...
	return: where condition, prefixed with table alias, also tables list, used by db_query to build correct join clause
	"""
	if not user: user = frappe.session.user
	...
	if not mandatory_auths:
		return "( 1 = 2 )"
	if not tables:  # reportview call get match condition without processing table joins, subfield can't be processed
	...
	for i in range(count):
		...
		if '.' in auth_field:  # handle 1 level link field's subfield
			...
			if link_table:
				...
				tables.append(table)
		field_condition = build_condition(auths[i], table2_alias, doctype, user)
