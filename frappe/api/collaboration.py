...
def add_favourite(doc: Document, body: dict) -> None:
	toggle_favourite(doc, add=True)


def remove_favourite(doc: Document, key: None) -> None:
	toggle_favourite(doc, add=False)


def toggle_favourite(doc: Document, add: bool) -> None:
	"""A favourite is a like. The read composes the part from the document's own column, which
	the toggle writes straight to the database, so the loaded document is refreshed from it."""
	toggle_like(doc.doctype, doc.name, add=add)
	doc._liked_by = frappe.db.get_value(doc.doctype, doc.name, "_liked_by")
