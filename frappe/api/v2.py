...
import nts
import nts.client
from nts import _, get_newargs, is_whitelisted
from nts.core.doctype.server_script.server_script_utils import get_server_script_map
from nts.handler import is_valid_http_method, run_server_script, upload_file
...
def handle_rpc_call(method: str, doctype: str | None = None):
	from nts.modules.utils import load_doctype_module
	...
	method = nts.override_whitelisted_method(method)
	...
	try:
		...
		nts.throw(_("Failed to get method {0} with {1}").format(method, e))
...
def document_list(doctype: str):
	...
	nts.form_dict.limit_page_length = nts.form_dict.limit or 20
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = nts.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def delete_doc(doctype: str, name: str):
	nts.client.delete_doc(doctype, name)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	doc.check_permission(PERMISSION_MAP[nts.request.method])
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	if isinstance(document, str):
		document = nts.parse_json(document)

	if kwargs is None:
		kwargs = {}

	doc = nts.get_doc(document)
	...
	nts.response.docs.append(doc)  # send modified document and result both.
	return response
