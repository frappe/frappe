...
SettionType: TypeAlias = _dict[str, Any]
# TODO: implement dataclass
LogMessageType: TypeAlias = _dict[str, Any]
# TODO: implement dataclass
JobMetaType: TypeAlias = _dict[str, Any]  # holds job metadata if the code is run in a background job context
...
if TYPE_CHECKING:  # pragma: no cover
	# trick becaues some type checkers fail to follow "RedisWrapper", etc (written as string literal)
	# trough a generic wrapper; seems to be a bug
	db: MariaDBDatabase | PostgresDatabase
	qb: MariaDB | Postgres
	conf: ConfType
	form_dict: _dict[str, str]
	request: Request
	session: SessionType
	user: str
	flags: _dict[str, Any]
