...
session: LocalProxy[_dict[str, Any]] = local("session")  # type: ignore[no-any-explicit]
user: LocalProxy[str] = local("user")
flags: LocalProxy[_dict[str, Any]] = local("flags")  # type: ignore[no-any-explicit]

error_log: LocalProxy[list[Any]] = local("error_log")
debug_log: LocalProxy[list[Any]] = local("debug_log")
message_log: LocalProxy[list[Any]] = local("message_log")
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	...
	local.locked_documents: list[str] = []
...
def get_cached_value(
	doctype: str, name: str, fieldname: str | list[str] = "name", as_dict: bool = False
) -> Any:
	try:
...
@request_cache
def _load_app_hooks(app_name: str | None = None) -> dict[str, Any]:
	import types
