...
nts.defaultDateFormat = "YYYY-MM-DD";
nts.defaultTimeFormat = "HH:mm:ss";
nts.defaultDatetimeFormat = nts.defaultDateFormat + " " + nts.defaultTimeFormat;
moment.defaultFormat = nts.defaultDateFormat;
...
$.extend(nts.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (nts.boot.time_zone && nts.boot.time_zone.system && nts.boot.time_zone.user) {
			date_obj = moment
				.tz(date, nts.boot.time_zone.system)
				.clone()
				.tz(nts.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(nts.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (nts.boot.time_zone && nts.boot.time_zone.system && nts.boot.time_zone.user) {
			date_obj = moment
				.tz(date, nts.boot.time_zone.user)
				.clone()
				.tz(nts.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (nts.boot.time_zone && nts.boot.time_zone.system && nts.boot.time_zone.user) {
			return (
				moment().tz(nts.boot.time_zone.system).utcOffset() ===
				moment().tz(nts.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return nts.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, nts.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(nts.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (nts.sys_defaults && nts.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (nts.sys_defaults && nts.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		...
		const user_date_fmt = nts.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = nts.datetime.get_user_time_fmt();

		if (only_time) {
			let date_obj = moment(val, nts.defaultTimeFormat);
			return date_obj.format(user_time_fmt);
		} else if (only_date || (typeof val === "string" && val.indexOf(" ") === -1)) {
			let date_obj = moment(val, nts.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			const system_datetime = moment.tz(
				val,
				nts.defaultDatetimeFormat,
				nts.boot.time_zone.system
			);
			const user_datetime = system_datetime.clone().tz(nts.boot.time_zone.user);
	...
	user_to_str: function (val, only_time = false) {
		var user_time_fmt = nts.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(nts.defaultTimeFormat);
		}

		var user_fmt = nts.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return nts.datetime.str_to_obj(nts.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return nts.datetime._date(nts.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return nts.datetime._date(nts.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return nts.datetime._date(nts.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return nts.datetime._date(nts.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = nts.boot.time_zone?.system || nts.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = nts.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? nts.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	validate: function (d) {
		return moment(
			d,
			[nts.defaultDateFormat, nts.defaultDatetimeFormat, nts.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = nts.sys_defaults.first_day_of_the_week || "Sunday";
