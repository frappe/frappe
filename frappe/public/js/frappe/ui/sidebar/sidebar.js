...
`);
	...
	// workspace route) and then stays put as you navigate. It moves automatically when you navigate
	// to an entity the current sidebar does NOT link, resolving the target in this order:
	//   - the entity's own module sidebar (sidebar_from_module) is preferred first, so a
	//     custom/standalone doctype lands in its module shell even when another sidebar links it.
	//   - otherwise the entity is followed to a sidebar that links it (get_workspace_sidebars).
	// If the current sidebar already links the entity, it stays put. Resolving from data (not the
	// DOM/location) keeps it independent of route/render timing, and it takes priority over the
	// cold-entry fallback. The active-item highlight stays route-aware via set_active_workspace_item().
	set_workspace_sidebar(router) {
		try {
			...
			if (route[0] === "Workspaces" && route.length >= 2) {
				...
				if (!this.sidebar_title) {
					// cold entry / deep link -> resolve once (resolve_initial_sidebar is module-first too)
					const target = this.initial_sidebar(route);
					if (target) frappe.app.sidebar.setup(target);
				} else if (in_current) {
					// the entity is already linked in the current sidebar -> stay put, don't yank away
				} else if (module_sidebar) {
					// prefer the entity's own module sidebar before following a link candidate, so a
					// custom/standalone doctype lands in its module shell even when another sidebar
					// happens to link it.
					if (module_sidebar.toLowerCase() !== this.workspace_title) {
						this.select_sidebar(module_sidebar);
					}
				} else if (candidates.length) {
					// no module sidebar -> follow the entity to a sidebar that links it
	...
	// Precedence:
	//   1. keep the last selected sidebar if the routed entity is linked in it (don't yank the user
	//      off a valid selection that already shows the entity)
	//   2. the entity's own module sidebar — checked BEFORE link candidates so a custom/standalone
	//      doctype lands in its module shell even when some unrelated sidebar happens to link it
	//   3. otherwise the first sidebar that links the entity
	...
	resolve_initial_sidebar(route) {
		...
		// 1. the routed entity is linked in the last selected sidebar -> keep it
		...
		// 2. prefer the entity's own module sidebar, before any link candidate
		if (module_sidebar) {
			return {
				sidebar: module_sidebar,
				reason: `derived from "${entity}"'s module — it has its own module sidebar`,
			};
		}

		// 3. no module sidebar -> follow the entity to a sidebar that links it
	...
	// The sidebar for the entity's module, or null. This is the preferred home for an entity: it
	// lands custom/standalone doctypes in their module shell even when some unrelated sidebar also
	// links them (every module has a sidebar -- authored, else autogenerated). The module's sidebar
	// is keyed by the module name, so we look it up directly rather than scanning for the first
	// sidebar carrying the module -- the latter varies per user as permission filtering changes
	// which sidebars survive and in what order. Returns null when the module can't be determined or
	// has no sidebar.
	//
	// For doctype routes (list/form) the router has already fetched the meta and cached it as
	// `frappe.router.meta`, so prefer that -- on a cold load `frappe.get_meta(entity)` can still be
	// empty, whereas the router's copy is loaded before this runs.
	sidebar_from_module(entity) {
		const router_meta = frappe.router.meta;
		const meta =
			router_meta && router_meta.name === entity
				? router_meta
				: entity && frappe.get_meta(entity);
