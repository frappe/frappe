import "./sidebar_item_v16";
import { SidebarEditor } from "./sidebar_editor";
frappe.ui.V16Sidebar = class V16Sidebar {
	constructor() {
...
`);
	...
	setup(workspace_title) {
		...
		this.sidebar_header = new frappe.ui.V16SidebarHeader(this);
		this.make_sidebar();
	...
	make_dom() {
		...
		this.wrapper = $(
			frappe.render_template("sidebar_v16", {
	...
	make_sidebar_item(opts) {
		...
		return new frappe.ui.v16_sidebar_item[class_name](opts);
	}
	...
	// Global search offers an "Open <X> Workspace" result (see search_utils.js), which is
	// shared by both navigation systems. Workspace navigation switches the sidebar and
	// jumps to its first item; there is no equivalent here, so route to the workspace and
	// let the usual route handling pick the sidebar up.
	open_workspace(name) {
