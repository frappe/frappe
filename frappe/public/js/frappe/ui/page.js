...
frappe.ui.Page = class FrappeAppPage {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_main_section() {
		...
		this.views['main'] = this.wrapper.find('.layout-main');
		this.set_view('main');
		this.setup_page();
	}
	...
	*/
	...
	set_top_alert(message, css_class='primary') {
		this.set_alert(this.wrapper.find('.page-top-alert'), message, css_class);
	}

	set_bottom_alert(message, css_class='primary') {
		this.set_alert(this.wrapper.find('.page-bottom-alert'), message, css_class);
	}

	set_alert($container, message, css_class) {
		if (message) {
			$container.removeClass('hide').find('.alert').removeClass().addClass(`alert alert-${css_class}`).html(message);
		} else {
