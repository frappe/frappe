...
frappe.tinymce = {
	add_simple: function(ele, height) {
		...
		// no create
		ele.myid = frappe.dom.set_unique_id(ele);
		$(ele).tinymce({
...
frappe.ele = {
	link: function(args) {
		...
		span.loading_img.src= 'assets/frappe/images/ui/button-load.gif';
...
function $ln(parent, label, onclick, style) { 
	return frappe.ele.link({parent:parent, label:label, onclick:onclick, style:style})
}
