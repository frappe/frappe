(function() {
	...
	class DataTable {
		...
		update_cell(row_index, col_index, value) {
			...
			const cell = row.find(cell => cell.col_index == col_index);
			cell.data = value;
			this.refresh_cell(cell);
		}
		...
		bind_events() {
			var me = this;
			this.body_scrollable.on('click', '.data-table-col', function() {
				me.body_scrollable.find('.data-table-col').removeClass('selected');
				const $col = $(this);
		...
		set_dimensions() {
			...
			this.body_scrollable.find('.table').css('margin', 0);
		}
		...
		bind_cell_double_click() {
			...
			const $edit_popup = this.wrapper.find('.edit-popup');
			...
			this.body_scrollable.on('dblclick', '.data-table-col', function() {
				...
				events.on_cell_doubleclick($cell.get(0), $edit_popup, row_index, col_index);
			});
		...
		get_row(row_index) {
			return this.rows.find(row => row[0].row_index == row_index);
		}
	}
	...
	function get_body(rows) {
		return `<tbody>
			${rows.html_map(row => get_row({ cells: row }))}
		</tbody>
		`
	}
	...
	function get_cell(cell) {
		...
		return `
			<td class="data-table-col noselect" ${custom_attr}>
	...
	Array.prototype.html_map = function(fn) {
