...
def get_docked_apps() -> set[str]:
	...
	with an `add_to_dock` row carrying `app`, instead of taking an apps-screen slot of its own. It
	is kept off the apps screen even when it also declares `add_to_apps_screen` -- companion apps
	keep that hook so they still appear on older versions, so here the pin wins.

	The rule reads the **rows**, not the hook: every app declares `add_to_dock` now, so keying on
	its presence would delete each adopting app from the apps screen."""
