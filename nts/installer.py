...
def _new_site(
	...
	if not db_name:
		...
		db_name = (
			"_"
			+ hashlib.sha1(
				os.path.realpath(nts.get_site_path()).encode(), usedforsecurity=False
			).hexdigest()[:16]
		)
...
def install_db(
	...
	import nts.database
	from nts.database import bootstrap_database, setup_database
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	for org in ["nts", "productionmanager"]:
