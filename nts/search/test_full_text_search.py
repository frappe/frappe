...
class TestFullTextSearch(ntsTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/productionmanager")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/productionmanager", "content": """Awesomeproductionmanager"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/productionmanager" not in res)

		res = self.index.search("Awesomeproductionmanager")
		self.assertEqual(res[0], "sw/productionmanager")

		# Update new doc
		self.index.update_index({"name": "sw/ntsbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/ntsbooks")
...
def get_index():
	return TestWrapper("test_nts_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/productionmanager",
			"content": """productionmanager is a free and open-source integrated Enterprise Resource Planning software developed by
			nts Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			productionmanager is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/nts",
			"content": """nts Framework is a full-stack web framework, that includes everything you need to build and
