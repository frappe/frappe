...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = nts.boot.time_zone
		? nts.boot.time_zone.user
		: nts.sys_defaults.time_zone;
...
<template>
	<div class="control nts-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="nts.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="nts.utils.icon('down', 'sm')"
		/>
	</div>
</template>
