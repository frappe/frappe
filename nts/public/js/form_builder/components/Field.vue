...
function duplicate_field() {
	...
	duplicate_field.df.owner = nts.session.user;
...
function make_dialog(frm) {
	...
	if (frm.doctype === "Customize Form") {
		...
		nts.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					nts.call({
						method: "nts.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	nts.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="nts.utils.icon('filter', 'sm')"></div>
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="nts.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="nts.utils.icon('move', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="nts.utils.icon('duplicate', 'sm')"></div>
					</button>
					<button
						class="btn btn-xs btn-icon"
