nts.provide("nts.model.user_settings");

$.extend(nts.model.user_settings, {
	get: function (doctype) {
		return nts
			.call("nts.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (nts.session.user === "Guest") return Promise.resolve();

		const old_user_settings = nts.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = nts.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		if (nts.session.user === "Guest") return Promise.resolve();
		return nts.call({
			method: "nts.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				nts.model.user_settings[doctype] = r.message;
			},
		});
	},
});

nts.get_user_settings = function (doctype, key) {
	var settings = nts.model.user_settings[doctype] || {};
	if (key) {
