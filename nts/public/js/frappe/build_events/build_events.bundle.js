...
nts.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						nts.assets._executed = nts.assets._executed.filter(
		...
		// update assets json
		nts.call("nts.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				nts.boot.assets_json = r.message;

				if (nts.hot_update) {
					nts.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
