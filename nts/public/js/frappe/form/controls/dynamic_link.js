nts.ui.form.ControlDynamicLink = class ControlDynamicLink extends nts.ui.form.ControlLink {
	get_options() {
		let options = "";
		if (this.df.get_options) {
			...
			options = nts.model.get_value(this.df.parent, this.docname, this.df.options);
		}

		if (nts.model.is_single(options)) {
			nts.throw(__("{0} is not a valid DocType for Dynamic Link", [options.bold()]));
		}
