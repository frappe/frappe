...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (nts.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: nts.markdown(this.frm.comment_box.get_value()),
				};
				return new nts.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				nts.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		nts.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	add_web_page_view_count() {
		if (this.frm.doc.route && cint(nts.boot.website_tracking_enabled)) {
			nts.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await nts.call({
			method: "nts.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = nts.utils.get_form_link("Communication", doc.name);
		...
		doc.content = nts.dom.remove_script_and_style(doc.content);
		let communication_content = $(nts.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = nts.dom.remove_script_and_style(doc.content);
		const comment_content = $(nts.render_template("timeline_message_box", { doc }));
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = nts.meta.get_label(this.frm.doctype, milestone_log.track_field);
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			custom_timeline_contents.push({
				...
				content:
					custom_item.content ||
					nts.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach((log) => {
			...
			energy_point_timeline_contents.push({
				...
				content: nts.energy_points.format_form_log(log),
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${nts.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${nts.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != nts.session.user_email
		...
		const email_accounts = nts.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(nts.session.user_email) &&
				communication_doc.sender != nts.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != nts.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = nts.markdown(this.frm.comment_box.get_value());
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (nts.session.user == doc.owner || nts.user.has_role("System Manager")) {
			if (nts.model.can_delete("Comment")) {
				const delete_option = $(`
		...
		let edit_button = $();
		let current_user = nts.session.user;
	...
	update_comment(name, content) {
		return nts
			.xcall("nts.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		nts.confirm(__("Delete comment?"), () => {
			return nts
				.xcall("nts.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	update_comment_publicity(comment_name, publish) {
		...
		nts.confirm(message, () => {
			return nts
				.xcall("nts.desk.form.utils.update_comment_publicity", {
					name: comment_name,
					publish,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = nts.urllib.get_full_url(
			nts.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		nts.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
