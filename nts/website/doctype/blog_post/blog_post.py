...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				nts.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				nts.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = nts.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			nts.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and nts.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = nts.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = nts.db.get_value(
		...
		context.guest_allowed = nts.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if nts.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = nts.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not nts.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = nts.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = nts.db.count("Comment", filters) or 0
		...
		context.like = nts.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	...
	blog_settings = nts.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = nts.utils.escape_html(
		nts.local.form_dict.blog_category or nts.local.form_dict.category
	)
	if category_name:
		category = nts.get_doc("Blog Category", category_name)
	...
	elif nts.local.form_dict.blogger:
		blogger = nts.db.get_value("Blogger", {"name": nts.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif nts.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(nts.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = nts.qb.DocType("Blog Post"), nts.qb.DocType("Blog Category")
	return (
		nts.qb.from_(category)
...
def clear_blog_cache():
	for blog in nts.db.get_list("Blog Post", fields=["route"], pluck="route", filters={"published": True}):
		clear_cache(blog)
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	if filters and filters.get("blog_category"):
		...
		category = nts.utils.escape_html(
			nts.local.form_dict.blog_category or nts.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % nts.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % nts.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(nts.db.escape("%" + txt + "%"))
		)

	if conditions:
		nts.local.no_cache = 1
	...
	posts = nts.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = nts.db.get_value(
