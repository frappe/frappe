...
def execute():
	...
	all_tables = nts.db.get_tables()
	final_deletion_map = nts._dict()

	for table in all_tables:
		indexes_to_keep_map = nts._dict()
		indexes_to_delete = []
		index_info = nts.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				nts.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
