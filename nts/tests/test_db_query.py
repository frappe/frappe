...
from nts.database.utils import DefaultOrderBy
from nts.desk.reportview import get_filters_cond
from nts.handler import execute_cmd
from nts.model.db_query import DatabaseQuery, get_between_date_filter
from nts.permissions import add_user_permission, clear_user_permissions_for_doctype
from nts.query_builder import Column
...
class TestDBQuery(ntsTestCase):
	...
	def test_child_table_field_syntax(self):
		note = nts.get_doc(
			doctype="Note",
			title=f"Test {nts.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		nts.delete_doc_if_exists("DocType", "Parent DocType 1")
		nts.delete_doc_if_exists("DocType", "Parent DocType 2")
		nts.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		nts.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		nts.db.delete("Parent DocType 1")
		nts.db.delete("Parent DocType 2")
		nts.db.delete("Child DocType")
		...
		# test query
		results1 = nts.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = nts.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = nts.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = nts.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = nts.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = nts.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = nts._dict(fieldtype="Date")
		datetime_df = nts._dict(fieldtype="Datetime")
		today = nts.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		nts.set_user("test2@example.com")
		self.assertRaises(nts.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(nts.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from nts.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = nts.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = nts.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = nts.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = nts.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(nts.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(nts.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		nts.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			nts.get_list("Prepared Report", ["*"])
			nts.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# nts.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = nts.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = nts.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		nts.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		nts.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		nts.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = nts.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from nts.desk.doctype.dashboard_settings.dashboard_settings import (
	...
	def test_virtual_doctype(self):
		...
		with patch(
			"nts.controllers",
			new={nts.local.site: {"Virtual DocType": VirtualDocType}},
		):
			VirtualDocType.get_list = MagicMock()

			nts.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", nts.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", nts.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", nts.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", nts.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", nts.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", nts.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", nts.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", nts.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			nts.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", nts.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from nts.desk.reportview import get

		if not nts.db.exists("DocType", "Related Todos"):
			nts.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			nts.db.delete("Related Todos")

		todo_one = nts.get_doc(
			{
				"doctype": "ToDo",
		...
		nts.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = nts.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], nts.db.count("Language"))
		self.assertEqual(count[1], nts.db.count("Language"))

	def test_ifnull_none(self):
		query = nts.get_all("DocField", {"fieldname": None}, run=0)
		self.assertIn("''", query)
		self.assertNotIn("\\'", query)
		self.assertNotIn("ifnull", query)
		self.assertFalse(nts.get_all("DocField", {"name": None}))
...
class TestReportView(ntsTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("nts.desk.reportview.get_count")
		nts.local.form_dict = nts._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("nts.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("nts.desk.reportview.get_count")
		current_value = nts.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("nts.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(nts.get_list("Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		nts.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		nts.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(nts.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			nts.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = nts.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_nested_doctype():
	if nts.db.exists("DocType", "Nested DocType"):
		return
