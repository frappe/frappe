...
import nts
from nts.model import core_doctypes_list, get_permitted_fields, is_default_field
from nts.model.utils import get_fetch_values
from nts.tests.utils import ntsTestCase


class TestModelUtils(ntsTestCase):
	def test_get_fetch_values(self):
		...
		full_name = nts.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = nts.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = nts.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = nts.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = nts.session.user or "Administrator"
	nts.set_user(user)
	yield
	nts.set_user(past_user)
