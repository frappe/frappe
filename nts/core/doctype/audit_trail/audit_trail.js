...
nts.ui.form.on("Audit Trail", {
	...
	render_changed_fields(frm, document_names, changed_fields) {
		...
		$(nts.render_template("audit_trail", render_dict)).appendTo(
	...
	render_rows_added_or_removed(frm, changed_fields) {
		...
		for (let key in added_or_removed) {
			...
			$(nts.render_template("audit_trail_rows_added_removed", section_dict)).appendTo(
