...
nts.ui.form.on("Data Import", {
	setup(frm) {
		nts.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			nts.model.clear_doc("Data Import", frm.doc.name);
			nts.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
	...
	show_import_status(frm) {
		nts.call({
			method: "nts.core.doctype.data_import.data_import.get_import_status",
			args: {
	...
	download_template(frm) {
		nts.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new nts.data_import.DataExporter(
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/nts.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/nts.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		nts.call({
			method: "nts.core.doctype.data_import.data_import.get_import_logs",
			args: {
				data_import: frm.doc.name,
			},
			callback: function (r) {
