...
from nts.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from nts.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			nts.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				nts.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not nts.db.get_single_value("SMS Settings", "sms_gateway_url"):
					nts.throw(
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = nts.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = nts.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = nts.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			nts.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		nts.cache.delete_value("system_settings")
		nts.cache.delete_value("time_zone")

		if nts.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from nts.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				nts.db.set_default(df.fieldname, self.get(df.fieldname))
...
def load():
	...
	all_defaults = nts.db.get_defaults()
	defaults = {}

	for df in nts.get_meta("System Settings").get("fields"):
...
def sync_system_settings():
	if nts.db.get_single_value("System Settings", "currency") is None:
		nts.db.set_single_value("System Settings", "currency", nts.defaults.get_defaults()["currency"])
