...
import nts
from nts import _
from nts.cache_manager import clear_controller_cache, clear_user_cache
...
from nts.database.schema import validate_column_length, validate_column_name
from nts.desk.notifications import delete_notification_count_for, get_filters_for
...
from nts.modules import get_doc_path, make_boilerplate
from nts.modules.import_file import get_file_path
from nts.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from nts.query_builder.functions import Concat
from nts.utils import cint, flt, get_datetime, is_a_property, random_string
from nts.website.utils import clear_cache

if TYPE_CHECKING:
	from nts.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(nts.ValidationError):
	pass


class UniqueFieldnameError(nts.ValidationError):
	pass


class IllegalMandatoryError(nts.ValidationError):
	pass


class DoctypeLinkError(nts.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(nts.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(nts.ValidationError):
	pass


class NonUniqueError(nts.ValidationError):
	pass


class CannotIndexedError(nts.ValidationError):
	pass


class CannotCreateStandardDoctypeError(nts.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		...
		from nts.core.doctype.docperm.docperm import DocPerm
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = nts.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			nts.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not (d.unique or d.search_index):
				referred_as_link = nts.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if nts.flags.in_patch or nts.flags.in_test:
			return

		if not nts.conf.get("developer_mode") and not self.custom:
			nts.throw(
		...
		if self.is_virtual and self.custom:
			nts.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if nts.conf.developer_mode and not self.owner:
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = nts.get_meta(nts.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except nts.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = nts.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				nts.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from nts.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = nts.bold(self.meta.get_field("max_attachments").label)
			nts.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if nts.flags.in_import:
			return
		parent_list = nts.get_all(
			"DocField", "parent", dict(fieldtype=["in", nts.model.table_fields], options=self.name)
		)
		for p in parent_list:
			nts.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			nts.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			...
			and (nts.conf.developer_mode or nts.flags.allow_doctype_export)
		)
		...
		nts.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(nts.session.user)

		if not nts.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({nts.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			nts.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not nts.request) or nts.flags.in_test or nts.flags.in_install
			nts.enqueue("nts.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = nts.scrub(self.name)
		scrubbed_module = nts.scrub(self.module)
		controller_path = nts.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from nts.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from nts.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and nts.session.user != "Administrator":
			nts.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			nts.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			nts.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			nts.db.sql(
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if nts.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(nts.scrub(old), nts.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		nts.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(nts.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{nts.scrub(new)}.json")
		current_data = nts.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			nts.db.table_exists(self.name)
			and nts.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = nts.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = nts.get_module_path(
				nts.scrub(self.module), "doctype", nts.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		...
		from nts.types.exporter import TypeExporter
		...
		if any(nts.get_hooks("export_python_type_annotations", app_name=app)):
			TypeExporter(self).export_types()

	def make_amendable(self):
		"""If is_submittable is set, add amended_from docfields."""
		if self.is_submittable:
			docfield_exists = nts.get_all(
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			...
			) and not nts.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = nts.bold(_("Parent Field (Tree)"))
			nts.throw(_("{0} is a mandatory field").format(field_label), nts.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			nts.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = nts.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = nts.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = nts.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			nts.throw(_("DocType's name should not start or end with whitespace"), nts.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(nts.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			nts.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(nts.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			nts.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		if used_in:
			nts.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			...
			if not property_value:
				return nts.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (
			...
			if dt.doctype == "Customize Form":
				nts.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if nts.get_meta(dt.name).issingle:
				return False

			if not nts.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	nts.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or nts.flags.in_patch or nts.flags.in_fixtures or nts.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			nts.throw(message, nts.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			nts.throw(message, nts.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = nts.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, nts.bold(link.table_fieldname), nts.bold(meta.name)
			)
			nts.throw(message, nts.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		nts.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		nts.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		nts.throw(msg, InvalidFieldNameError)
...
def validate_fields(meta: Meta):
	...
	:param meta: `nts.model.meta.Meta` object to check."""
	...
	def check_link_table_options(docname, d):
		if nts.flags.in_patch or nts.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = nts.db.get_value("DocType", d.options, "name")
				if not options:
					nts.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not nts.flags.in_migrate:
			nts.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			nts.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			nts.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and nts.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = nts.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					nts.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						nts.throw(_("Fold must come before a Section Break"))
				else:
					nts.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				nts.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			nts.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			nts.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			nts.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			nts.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			nts.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			nts.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			nts.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				nts.throw(_("Invalid {0} condition").format(nts.unscrub(field)), nts.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			nts.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = nts.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				nts.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if nts.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = nts.get_meta(doctype)

		if not child_doctype_meta.istable:
			nts.throw(
				_("Option {0} for field {1} is not a child table").format(
					nts.bold(doctype), nts.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			nts.throw(f"Max for {nts.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				nts.throw(_("Options for Rating field can range from 3 to 10"))
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = nts.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		nts.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			nts.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			nts.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			nts.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			nts.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			nts.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			nts.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			nts.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			nts.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			nts.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				nts.throw(
					...
					).format(d.idx, nts.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in nts.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not nts.db.exists("Domain", doc.restrict_to_domain)
		):
			nts.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in nts.db.get_tables() and not nts.db.exists("Module Def", doc.module):
			m = nts.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if nts.scrub(doc.module) in nts.local.module_app:
				m.app_name = nts.local.module_app[nts.scrub(doc.module)]
			else:
				m.app_name = "nts"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if nts.flags.package:
				m.package = nts.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if nts.db.table_exists("Role", cached=False) and not nts.db.exists("Role", role):
				r = nts.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
...
def check_fieldname_conflicts(docfield):
	...
	doc = nts.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		nts.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	nts.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		nts.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		nts.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		nts.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		nts.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	try:
		return flt(nts.db.get_row_size(doctype) / nts.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
