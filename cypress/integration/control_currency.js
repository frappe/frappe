	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("nts")
				.then((nts) => {
					nts.boot.sysdefaults.currency = test_case.currency;
					nts.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					nts.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
