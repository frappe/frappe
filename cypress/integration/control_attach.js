context('Date Control', () => {
	...
	it('Checking functionality for "Link" button in the "Attach" fieldtype', () => {
		...
		cy.get('.control-input > .btn-sm').contains('Attach').click();

		//Clicking on "Link" button to attach a file using the "Link" button
		cy.get('.mt-2').findByRole('button', {name: 'Link'}).click();
		...
		cy.get('.modal-footer > .standard-actions > .btn-primary').contains('Upload').click({delay: 500});
		cy.wait(800);
	...
	it('Checking functionality for "Library" button in the "Attach" fieldtype', () => {
		...
		cy.get('.mt-2').findByRole('button', {name: 'Library'}).click();
