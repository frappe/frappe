context("Child Table Data Import", () => {
	...
	it("stops on Fix Issues when a cell fails validation", () => {
		...
		dialog().find(".grid-import-pending-cell").should("exist");
	...
	it("updates the row matching the ID without adding rows", () => {
		cy.get("@firstRowId").then((id) => {
			open_import("Update Existing Records");
			upload([`${id},9876500010,1`, "no-such-row,9876500011,0"], `ID,${HEADER}`);

			dialog().find(".grid-import-preview-row.has-note").should("exist");
			go_to_step("Preview");
			primary("Apply");
			...
			phone_rows().then((rows) => {
				...
				expect(rows.map((d) => d.phone)).to.not.include("9876500011");
			});
		});
	});
	...
	it("does not map a column whose permlevel the user cannot write", () => {
		...
		dialog().find('th[data-col="1"]').should("have.class", "has-error");
		dialog()
			.find('.grid-import-mapping-row td[data-col="1"] input')
			.should("have.value", "Don't Import")
			.click();
