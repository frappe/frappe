...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((nts) => {
				return nts.xcall(
					"nts.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("nts")
			.then((nts) => {
				nts.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.nts-control[data-fieldname="child_table_1"]').as("table");
