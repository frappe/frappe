...
context("Print Format Builder — selection & spacing", () => {
	...
	it("bulk drag keeps the selected group contiguous and ordered", () => {
				...
				columns: [
					{
						...
						fields: [
							...
							{ fieldtype: "Data", fieldname: "status", label: "Status" },
							{ fieldtype: "Data", fieldname: "priority", label: "Priority" },
							{ fieldtype: "Data", fieldname: "footer", label: "Footer" },
						],
					},
				],
			},
		...
		cy.get("[data-pfb-section] .field", { timeout: 30000 }).should("have.length", 4);
		cy.get("[data-pfb-section] .field").eq(0).click({ force: true });
		cy.get("[data-pfb-section] .field").eq(1).click({ shiftKey: true, force: true });
		cy.get(".field--selected").should("have.length", 2);

		// mimic dropping the grabbed field at the column end: SortableJS moves only
		// that one field, then our drop handler pulls the rest of the group over
		cy.get(".builder-root").then(($r) => {
			const provided = $r[0].__vueParentComponent.provides["$store"];
			const store = provided.value ?? provided;
			const col = store.layout.value.sections[0].columns[0];
			const group = store
				.ordered_body_fields()
				.filter((f) => store.selected_fields.value.includes(f));
			const dragged = group[0];
			col.fields.splice(col.fields.indexOf(dragged), 1);
			col.fields.push(dragged);
			store.reflow_dragged_group(dragged, group);
			store.set_selected(group);

			const order = col.fields.filter((f) => !f.remove).map((f) => f.label);
			expect(order).to.deep.equal(["Priority", "Footer", "Description", "Status"]);
			expect(store.selected_fields.value.map((f) => f.label).sort()).to.deep.equal([
				"Description",
				"Status",
			]);
		});

		// the group stays selected after the move
		cy.get(".field--selected").should("have.length", 2);
	});
});
