...
context("Print Format Builder — create flow", () => {
	...
	it("redirects to the Print Format list when opened without a format", () => {
		cy.visit("/desk/print-format-builder-beta");
	...
	it("left panel opens on Fields and remembers the tab you left on", () => {
		...
		cy.visit(`/desk/print-format-builder-beta/${encodeURIComponent(PF_NAME)}`);
...
context("Print Format Builder — draft and Save & Apply", () => {
	...
	it("keeps Typst block markup through Save & Apply", () => {
		...
		cy.visit(`/desk/print-format-builder-beta/${encodeURIComponent(TYPST_PF_NAME)}`);
