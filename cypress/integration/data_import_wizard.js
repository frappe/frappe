context("Data Import Wizard", () => {
	...
	it("does not leak attached file across documents and preserves source visibility parity", () => {
		...
		cy.get(".diw-fui-tab").contains("Google Sheet").click({ force: true });
	...
	it("keeps the Import step locked until import has started", () => {
		...
		cy.contains(".diw-stepper .diw-step", /^Import$/)
	...
	it("locks source fields and CSV controls after finished imports", () => {
		...
		wizard_import_file_field().contains("Clear").should("not.exist");
		wizard_import_file_field().contains("Attach").should("not.exist");
