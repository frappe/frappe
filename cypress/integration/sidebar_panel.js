...
context("Sidebar Panel", () => {
	...
	it("carries both triggers on the sidebar, docked app or not", () => {
		...
		cy.get(".standard-items-band .sidebar-background-tasks").should("exist");
		cy.get(".standard-items-band .sidebar-notification").should("exist");
		cy.get(".dock .sidebar-background-tasks").should("not.exist");
		cy.get(".dock .sidebar-notification").should("not.exist");
	});

	it("opens the background tasks panel from the sidebar", () => {
		// Forced because the button is hidden on a site with no tasks, and this is about the
		// wiring from the button to the registry, not about the button's visibility.
		cy.get(".standard-items-band .sidebar-background-tasks").click({ force: true });
