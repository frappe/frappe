...
await frappe.ui.mount_island("insights.dashboard", el, {
	...
	"onUpdate:title": (title) => page.set_title(title),
});
```
...
everything but `name` and `context` passes through verbatim, so `v-model:title`
and `@navigate` work on it as they do on any component.
