  ...
  <div class="flex h-full min-h-0 flex-col">
    ...
    >
      ...
      <template #editor="{ editor: editorInstance }">
        <div class="flex min-h-0 flex-1 flex-col overflow-y-auto px-2.5">
...
// session.
const quotedStorageKey = props.storageKey
  ? `${props.storageKey}:quoted-reply`
  : null;
...
watch(
  ...
  (next, previous) => {
    const untouched =
      ...
      textOf(body.value) === textOf(withSignature(previous));
