...
export function getNumberFormatInfo(format: string): NumberFormatInfo {
  ...
  const precision = decimalStr ? format.split(decimalStr).slice(1)[0]?.length ?? 0 : 0
...
export function flt(value: unknown, options: FltOptions = {}): number {
  const { precision, numberFormat = DEFAULT_NUMBER_FORMAT, roundingMethod } = options
  ...
  if (typeof value === 'number') {
    ...
    if (s.indexOf(' ') !== -1) {
      const parts = s.split(' ')
      s = isNaN(parseFloat(parts[0])) ? parts.slice(parts.length - 1).join(' ') : s
    }
...
export function formatCurrency(value: unknown, options: FormatCurrencyOptions = {}): string {
  const { numberFormat = DEFAULT_NUMBER_FORMAT, currency, roundingMethod } = options
  const precision = options.precision ?? 2

  const number = formatNumber(value, { numberFormat, precision, roundingMethod })
 ...
 */
export function formatField(value: unknown, options: FormatFieldOptions = {}): string {
  if (value == null || value === '') return ''
  const { fieldtype, precision, currency, numberFormat, roundingMethod } = options
  switch (fieldtype) {
    case 'Int':
    ...
    case 'Currency':
      return formatCurrency(value, { numberFormat, currency, precision, roundingMethod })
    case 'Percent':
...
function roundNumber(num: number, precision: number, roundingMethod?: string): number {
  ...
  if (method === "Banker's Rounding (legacy)") {
    ...
    n = Math.abs(decimalPart - 0.5) < epsilon
      ? floorNum % 2 === 0
        ? floorNum
        : floorNum + 1
      : Math.round(n)
