...
describe('formatField', () => {
  it('formats Int as a plain integer — no grouping, no decimals (matches Frappe cint)', () => {
    ...
    expect(formatField(1234567, { fieldtype: 'Int', numberFormat: '#.###,##' })).toBe(
      '1234567',
    )
  })
  ...
  it('formats Currency with a resolved symbol', () => {
    expect(formatField(1234.5, { fieldtype: 'Currency', currency: 'USD' })).toBe(
      '$ 1,234.50',
    )
  })
  ...
  it('appends % for Percent', () => {
    expect(formatField(42.5, { fieldtype: 'Percent', precision: 1 })).toBe('42.5%')
  })
