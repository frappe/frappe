	...
	>
		<div class="flex items-center justify-between gap-2">
			<div class="flex items-center gap-1">
				<span>{{ email.senderFullName || "Guest" }}</span>
				<span v-if="email.sender" class="hidden text-sm text-ink-gray-5 sm:flex">
					{{ "<" + email.sender + ">" }}
				</span>
			</div>
		...
		<div class="text-p-sm text-ink-gray-5">
			<template
				v-for="(val, label) in { To: email.to, cc: email.cc, bcc: email.bcc }"
				:key="label"
			>
		...
		<EmailContent :content="email.content" :css-href="cssHref" />
...
const status = computed(() => {
	const _status = props.email.deliveryStatus;
	let color = "red";
...
const reply = () => {
	...
	emit("reply", {
		...
		to: user === props.email.sender ? props.email.to : props.email.sender,
	});
};

const replyAll = () => {
	const user = props.currentUser;
	const exclude = [user, props.email.sender];
	const filteredTo = splitRecipients(props.email.to, exclude);
	const filteredCc = splitRecipients(props.email.cc, exclude);
	const filteredBcc = splitRecipients(props.email.bcc, exclude);

	let _to: string;
	let _cc: string[];
	let _bcc: string[];

	if (user === props.email.sender) {
