<template>
	<template v-if="listConfig && list.data">
		<div class="mx-5 my-4 flex h-[42rem] flex-col gap-4">
...
const createConfigObj = async () => {
	listConfig.value = {
		...
		sort: [
			config_settings.data.sort_field || "modified",
			config_settings.data.sort_order || "DESC",
		],
	}
...
const currentSort = computed(() => [
	listConfig.value.sort[0] || "modified",
	listConfig.value.sort[1] || "DESC",
])
	...
	list.data?.map(async (row) => {
		for (let fieldname in row) {
			...
			if (fieldtype === "Link" && row[fieldname]) {
				...
				let new_value = await call(
					"frappe.desk.doctype.view_config.view_config.get_link_title_field",
					{
						doctype: dt,
						name: row[fieldname],
					}
