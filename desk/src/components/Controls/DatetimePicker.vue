<template>
  <Popover @open="selectCurrentMonthYear" class="flex w-full [&>div:first-child]:w-full">
    <template #target="{ togglePopover }">
      <Input readonly type="text" :placeholder="placeholder" :value="value && formatter ? formatter(value) : value"
        @focus="!readonly ? togglePopover() : null" :class="inputClass" v-bind="$attrs" />
    </template>
    <template #body="{ togglePopover }">
      <div
        ...
        <div class="flex items-center justify-center gap-1 p-1">
          <TextInput class="text-sm" type="text" :value="value"
            @change="updateDate($event.target.value) || togglePopover()" />
          <Button :label="'Now'" class="text-sm" @click="selectDate(getDate(), false, true) || togglePopover()" />
        </div>
        <div class="flex flex-col items-center justify-center p-1 text-gray-800">
          <div class="flex items-center text-xs uppercase">
            <div class="flex h-6 w-8 items-center justify-center text-center"
              v-for="(d, i) in ['su', 'mo', 'tu', 'we', 'th', 'fr', 'sa']" :key="i">
              {{ d }}
            </div>
          </div>
          <div class="flex items-center" v-for="(week, i) in datesAsWeeks" :key="i">
            <div v-for="date in week" :key="toValue(date)"
              class="flex h-8 w-8 cursor-pointer items-center justify-center rounded hover:bg-gray-50" :class="{
        ...
        <div class="flex items-center justify-around gap-2 p-1">
          ...
          <div class="flex flex-col items-center justify-center">
            <div class="slider flex min-h-4 items-center justify-center">
              <TextInput v-model="hour" name="hours" type="range" min="0" max="23" step="1"
                @change="() => changeTime() || togglePopover()" />
            </div>
            <div class="slider flex min-h-4 items-center justify-center">
              <TextInput v-model="minute" name="minutes" type="range" min="0" max="59" step="1"
                @change="() => changeTime() || togglePopover()" />
            </div>
            <div class="slider flex min-h-4 items-center justify-center">
              <TextInput v-model="second" name="seconds" type="range" min="0" max="59" step="1"
                @change="() => changeTime() || togglePopover()" />
            </div>
          </div>
        </div>
        <div class="flex justify-end p-1">
          <Button :label="'Clear'" class="text-sm" @click="() => {
      selectDate('')
      togglePopover()
    }
    " />
