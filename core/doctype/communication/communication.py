...
def get_user(doctype, txt, searchfield, start, page_len, filters):
	...
	return webnotes.conn.sql("""select name, concat_ws(' ', first_name, middle_name, last_name) 
			...
			(%(key)s like "%(txt)s" or concat_ws(' ', first_name, middle_name, last_name) 
				like "%(txt)s")
			 %(mcond)s limit %(start)s, %(page_len)s """ % {'key': searchfield, 
...
def get_lead(doctype, txt, searchfield, start, page_len, filters):
	...
	return webnotes.conn.sql(""" select name, lead_name from `tabLead` 
			where docstatus < 2 and (%(key)s like "%(txt)s" 
				or lead_name like "%(txt)s" or company_name like "%(txt)s") 
			%(mcond)s order by lead_name asc limit %(start)s, %(page_len)s 
