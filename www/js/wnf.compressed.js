...
return span;},button:function(args){var css='button';if(args.color)css+=' '+args.color;var btn=$a(args.parent,'button',css);btn.loading_img=$a(args.parent,'img','',{margin:'0px 4px -2px 4px',display:'none'});btn.loading_img.src='images/ui/button-load.gif';if(args.is_ajax)$y(btn,{marginRight:'24px'});btn.innerHTML=args.label;btn.user_onclick=args.onclick;btn.onclick=function(){if(!this.disabled)this.user_onclick(this);}
btn.set_disabled=function(){this.disabled=1}
btn.set_enabled=function(){this.disabled=0;}
...
function $btn(parent,label,onclick,style,color,is_ajax){return wn.ele.button({parent:parent,label:label,onclick:onclick,style:style,is_ajax:is_ajax,color:color})}
...
var def_ph_style={wrapper:{marginBottom:'16px',backgroundColor:'#EEE'},main_heading:{},sub_heading:{marginBottom:'8px',color:'#555',display:'none'},separator:{borderTop:'3px solid #444'},toolbar_area:{padding:'3px 0px',display:'none',borderBottom:'1px solid #AAA',height:'27px'}}
...
this.lhs=$td(this.t1,0,0);this.main_head=$a(this.lhs,'h1','',def_ph_style.main_heading);this.sub_head=$a(this.lhs,'h4','',def_ph_style.sub_heading);this.separator=$a(this.wrapper,'div','',def_ph_style.separator);this.toolbar_area=$a(this.wrapper,'div','button-group',def_ph_style.toolbar_area);this.padding_area=$a(this.wrapper,'div','',{padding:'3px'});$y($td(this.t1,0,1),{textAlign:'right',padding:'3px'});this.close_btn=$btn($td(this.t1,0,1),'Close',function(){nav_obj.show_last_open();},0);this.close_btn.className='button pill icon remove';if(main_text)this.main_head.innerHTML=main_text;if(sub_text)this.sub_head.innerHTML=sub_text;this.buttons={};this.buttons2={};}
PageHeader.prototype.add_button=function(label,fn,bold,icon,green){var tb=this.toolbar_area;if(this.buttons[label])return;var btn=$btn(tb,label,fn,null,(green?'green':''));if(bold)$y(btn,{fontWeight:'bold'});this.buttons[label]=btn;$ds(this.toolbar_area);return btn;}
...
var uploaders={};var upload_frame_count=0;Uploader=function(parent,args,callback,debug){var id='frame'+upload_frame_count;upload_frame_count++;this.callback=callback;var div=$a(parent,'div');if(debug){var dims={width:'400px',height:'200px'}}else{var dims={width:'0px',height:'0px'}}
div.innerHTML=repl('<iframe id="'+id+'" name="'+id+'" src="blank1.html" \
 style="width:%(width)s; height:%(height)s; border:0px"></iframe>',dims);var div=$a(parent,'div');div.innerHTML='<form method="POST" enctype="multipart/form-data" action="'+outUrl+'" target="'+id+'"></form>';var ul_form=div.childNodes[0];var f_list=[];var inp_fdata=$a_input($a(ul_form,'span'),'file',{name:'filedata'},{marginLeft:'7px'});var inp=$a_input($a(ul_form,'span'),'hidden',{name:'cmd'});inp.value=args.cmd||'uploadfile';var inp=$a_input($a(ul_form,'span'),'hidden',{name:'uploader_id'});inp.value=id;var inp=$a_input($a(ul_form,'span'),'submit',null,{marginLeft:'7px'});inp.className='button';inp.value='Upload';$y(inp,{width:'80px'});for(var key in args){if(key!='cmd'){var inp=$a_input($a(ul_form,'span'),'hidden',{name:key});inp.value=args[key];}}
uploaders[id]=this;}
