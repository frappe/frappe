<!-- The seam on a column's side: drag a fixed column to resize, drag past the minimum to
     collapse, click for the same toggle the round chevron gives. -->
<template>
	<div class="group/edge relative z-20 w-0 shrink-0">
		<div
			...
			:class="[
				...
				!bounds ? '' : side === 'left' ? 'cursor-w-resize' : 'cursor-e-resize',
			]"
			@click="onClick"
		...
		<button
			...
			:aria-label="`${open ? 'Collapse' : 'Expand'} ${name} column`"
			:aria-expanded="open"
			@click="emit('toggle')"
		>
			<span
...
const props = defineProps<{
	...
	open: boolean;
	width: number;
	/** Absent on a flex column, whose edge only toggles. */
	bounds?: ColumnBounds;
	collapsible: boolean;
}>();
