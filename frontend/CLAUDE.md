...
`frontend/package.json` and `frontend/yarn.lock` do not exist in a clean checkout. Both are
**generated by `bench build`**: the package file from `package.base.json` plus each app's
`<app>/<app>/desk.package.json`, the lock from `yarn.lock.base` plus whatever those apps
add. Both are gitignored along with `manifest.json`, `node_modules`, `auto-imports.d.ts`,
`components.d.ts` and `frappe/public/frontend`.

So the suite does not run until you make the pair:
...
**99 files / 1187 tests**, under `recordPage/tests/`, `pages/record/tests/`,
`pages/record/panel/tests/`, `pages/record/tabs/tests/`, `shell/tests/` and `navigation/tests/`.
...
- **`frontend/yarn.lock.base` is committed and `frontend/yarn.lock` is generated.** The
  base pins the framework's transitive versions on every bench; the generated lock grows
  only what the bench's apps declare. After editing `package.base.json`, run
  `frontend/base-lock.sh` (or `yarn base-lock`) to refresh the base; it reads nothing
